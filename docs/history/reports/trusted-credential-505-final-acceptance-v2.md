...
> `scripts/trusted-credential-<uid>-final-v2-run.mjs`（root 编排，控制面
> `sudo -u <svc-user>` 以 <uid> 运行，child 经冻结 spawn helper 降权 <uid>/20）
> 证据：`.demo/trusted-credential-<uid>-v2/evidence.md`（20/20 checks + CP/child
> stderr 诊断尾巴；`.demo` 为 gitignored 运行时区）

## 0. 一句话总结

冻结架构的**最终特权边界验收全部通过**：Router/control plane 以
<svc-user>/<uid> 运行，DSH Agent child 经 root-owned setuid helper（冻结契约：
仅 <uid>/20、清 supplemental groups、execv 固定 argv、无 shell）降权到
<uid>/20；credential store 落在 <uid>-private zone（`~/.openclaw/credentials/`
<svc-user> 0700，store 文件 <svc-user> 0600），uid <uid> 三种读法全部 DENIED。
...
- `/usr/local/libexec/dsh-agent-spawn-helper`（root:wheel 4755，源码
  `scripts/dsh-agent-spawn-helper.c`）：只接受冻结 child uid/gid <uid>/20；
  `setgroups(0,NULL)` 清 supplemental groups；setgid→setuid→execv 固定
  argv；冒烟测试（降权 id 输出 groups=20(staff)、拒绝 501/80、<uid> 调用
  无增益）全部通过。
- `<home>/.openclaw/credentials/dsh-agent-core/`（<svc-user>:<svc-user>
  0700）：<uid>-private 验收 store 目录；store 文件 0600 <svc-user>-owned。
- 未新建任何 daemon / IPC / 通用 sudo 框架 / Auth。

## 2. 与 V1 验收的差异（Task 3 强化）

V1 的 A/B 共用同一个真实 MachineClient（knowledge-curator-agent）；
V2 从 <uid>-private zone（`agent-arch-reviewer-secret`）+ auth-service DB
（machine_clients × machine_principals）配对出第二个真实凭据：
**B = arch-reviewer**（`mc_oc_<redacted>`，
...
- 诊断期间形状侦察脚本曾打印 <uid> zone 内 secret 文件前 40 字符前缀到
