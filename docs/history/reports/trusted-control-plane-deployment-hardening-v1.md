...
> 关闭最后一个生产安全缺口：可信 Control Plane（uid <uid> = <svc-user>）执行的全部
> 降权前代码/配置迁入受保护 install，uid <uid>（Agent/用户）不可修改、不可替换、
> 不可用 symlink 指回 <uid>-writable 的开发 repo。
...
<uid> 控制面启动链（TRUSTED_CREDENTIAL_BROKER_V1 冻结形态）：

```
root 驱动（sudo）
  └─ sudo -u <svc-user> node <trusted>/harness/apps/cli/lib/bin.js --profile agent-core-integration
       ...
       ├─ config = <trusted>/config            （Agent Definition agents.json / bindings / credential store，<svc-user> 0700）
       └─ 子进程：<trusted>/libexec helper (root:wheel 4755) → setuid(<uid>) → exec <trusted>/harness CLI
            （降权后才执行；child profile/farm 指向 <uid> 可读的 trusted app 或 <uid> 自有区）
```
...
| 代码/配置 | 闭包 | 安装后属主 |
|---|---|---|
| DSH CLI + harness runtime | harness 源码拷贝 + `pnpm install --offline --frozen-lockfile --ignore-scripts --config.package-import-method=copy`（copy 模式，无指向 <uid> store 的硬链接） | <svc-user>:<svc-user> 0755/0644 |
| Agent Core 控制面代码 | app/：packages/*（src+package.json）、bundle-*、profile-*、scripts（resident/demo-home/…）；node_modules/@deepseek-ai → ../../harness/node_modules/.pnpm/node_modules/@deepseek-ai（trusted 内） | <svc-user>:<svc-user> 0755/0644 |
| 控制面 home | home/（profile 拷贝 + farm → ../app/…） | <svc-user>:<svc-user> |
| 生产配置 | config/（Agent Definition agents.json/bindings/credential store；模型设置源 <home>/.dsh 0600） | <svc-user>:<svc-user> 0700/0600 |
| spawn helper | /usr/local/libexec/dsh-agent-spawn-helper | root:wheel 4755 |

关键结论：
- 任何受保护入口都不会跳回 <home>（install 审计 + 运行时 lsof 断言：<uid> 进程
  打开 0 个 dev-repo 文件）；
- harness 闭包中无指向 <uid> 属主 store 的硬链接（package-import-method=copy）；
- child 侧（<uid> 降权后）继续执行 <uid>-writable 代码（child profile/farm 在 <uid> 区，
  workspace/runtime 普通权限）——符合"只有降权后才允许执行 <uid>-writable code"。
...
- `scripts/trusted-cp-deploy-install.sh`（root 运行，可重复）：
  ...
  4. 控制面 home（profile + farm）；种子 <home>/.dsh（模型设置源，0600）；
  5. config 0700 <svc-user>；6) 属主/模式；7) helper 校验 root:wheel 4755；
  8. symlink 审计（不允许逃出 trusted root）；9) 代码中无 <home> 引用审计；
  10) uid-<uid> 现场抽查（写/替换/symlink 全部 DENIED）。
- 无新 Deployment Platform；开发 repo / harness 保持 <uid>-writable 可继续开发。

## 3. Task 3 — DSH / Harness 边界

- <uid> parent 执行：<trusted>/harness 的 CLI + 全部组合插件 + <trusted>/app 的
  Agent Core 包 → 全部 trusted（<uid> 只读）。
- <uid> child 执行：同一 trusted CLI（降权后 exec，只读）＋ child 自有 profile/farm
  （<uid> 区，普通权限）→ 合规。
- helper：<uid> → helper(root) → setuid(<uid>) → exec DSH：exec 时已降权，child 代码
  可保持 <uid> 属主。
...
### Task 4 攻击矩阵（uid <uid>，canary 文件；run1 已全过，run2 复跑）
...
| symlink 把 trusted path 指回 <uid> repo（ln -s ~/dsh-agent-core app/packages/agent-router） | DENIED |
...
`real Agent(child <uid>, 无 secret/token) → parent RPC → trusted gateway(<uid>) →
real auth-service → real svc-forum → ok`；PARENT_UID=<uid>、CHILD_UID=<uid>、
CREDENTIAL_STORE_ACCESS_FROM_502=DENIED；restart 后 gateway 再起 + 再次真实调用。


## 5. Review 修复 — Trusted Node（MERGE_REVIEW blocker）

Review 唯一 blocker：<uid> pre-drop 的 Node interpreter 曾是 `/usr/local/bin/node`
（Homebrew，uid <uid> 可修改 → <uid> 换 node → restart CP → 以 <uid> 执行 → 边界绕过）。
...
- `TRUSTED_NODE = /usr/local/libexec/agent-core/node-runtime/bin/node`
  （<svc-user>:<svc-user> 0755/0644，<uid> 只读）；
- 安装时硬校验：真实文件（非 symlink）、可运行（--version）、**与 Cellar binary
  inode 不同**（非 hardlink）、node-runtime 内零 symlink。

### Fix 2 — 启动链统一（driver/部署）

- CP 启动：`sudo -u <svc-user> /usr/bin/env … <TRUSTED_NODE> <trusted-cli> --profile …`
  （不再出现 /usr/local/bin/node）；
- CP env：`PATH = <node-runtime>/bin:` 置首 —— 任何 <uid> pre-drop 的裸 `node`
  解析都落在 trusted runtime；
- child：helper argv 的 interpreter 来自 CP 的 `process.execPath` = trusted node
  （setuid(<uid>) 后执行同一 trusted runtime，读取路径 <uid> 只读）。

### Fix 3 — 攻击验收（新增，全部 PASS）

| 检查 | 结果 |
|---|---|
| TRUSTED_NODE_502_WRITABLE（<uid> 写 trusted node） | NO (DENIED) |
| TRUSTED_NODE_502_REPLACE_DENIED（<uid> rm node） | DENIED |
| TRUSTED_NODE_PARENT_502_WRITABLE（<uid> 写 node-runtime/bin） | NO (DENIED) |
| TRUSTED_NODE_PARENT_502_REPLACE_DENIED（<uid> rm node-runtime） | DENIED |
| TRUSTED_NODE_502_SYMLINK_REDIRECT_DENIED（ln -s /usr/local/bin/node 替换） | DENIED |
| TRUSTED_NODE_REAL_FILE（非 symlink，<svc-user> 0755） | PASS |
| TRUSTED_NODE_NO_CELLAR_HARDLINK（inode 独立） | PASS |
| restart 后 CP_NODE_PATH = trusted path、PARENT_UID=<uid> | PASS |
| <uid> lsof：0 个 /usr/local/Cellar/node、/usr/local/bin/node 文件 | PASS |
| <uid> lsof：trusted node 打开（1） | PASS |

### Fix 4 — 重跑 smoke（全部 PASS）

REAL_BROKER_SMOKE（child <uid> → Router → Broker → real auth → real svc-forum）、
RESTART_HARDENED、CHILD_UID=<uid>、CHILD_NO_CREDENTIAL、
...
- 安全边界零改动：TRUSTED_NODE / <uid>-<uid> / credential store / broker smoke /
...
- 迁移期间 find/修复的最新 main 架构差异（均机械兼容，非重新设计）：
  ...
  2. latest main 的 workspace-bootstrap（WORKSPACE_BOOTSTRAP_ROUTER_HOOK_V1）
     会让 <uid> parent 因 child workspace 的 AGENTS.md 权限报 EACCES → verify
  ...
  3. **acceptance-only 模型切换**：默认模型路由（pi-ai catalog 的
     ...
     install 脚本或 `<home>/.dsh`；产品默认模型配置零改动
...
`forum_my_notifications` → parent @<uid> → Broker → real auth-service →
