...
status: historical
as_of: 2026-08-15
superseded_by: ../../security/credentials.md
...
>
> Current documentation: [docs/security/credentials.md](../../security/credentials.md)

# TRUSTED_CREDENTIAL_BROKER_INTEGRATION_V1

> 状态：已完成实现 + 真实验收；**结论 = BLOCKED**（唯一阻塞 = 一次性 root
> bootstrap：本环境无法把 Router parent 以 uid <uid> 运行、无法把凭据 store 变为
> <uid>-owned —— 二者同源，代码与验收全部就绪，root 就位后同一脚本直接转 PASS）
...
```text
DSH Agent child (uid <uid>, NO credential, NO token)
  → existing parent RPC (agentRpc → demo-server → Router.onRpcRequest)
  → trusted Router parent (deployment uid <svc-user>/<uid>)
  → trusted Broker / credential boundary (in-process gateway, <uid>)
...
| parent 从 <uid>-private store 读取对应 MachineClient credential | ✅ `packages/broker/src/credential-store.js`（0600、fail-loud、每次调用重读支持轮换）；store 值永不进 child env/fs/wire/模型 |
...
| Router spawn DSH child 降权到普通 Agent runtime uid/gid（<uid>，非 per-Agent OS user） | ✅ `childSpawnConfig()`：DSH_AGENT_CHILD_UID/GID + DSH_AGENT_SPAWN_HELPER seam（<uid>→<uid> 需要 setuid-root helper，同属一次性 root bootstrap）；PER_AGENT_OS_USER = NO |
...
- `packages/broker/src/credential-store.js`（新）：<uid>-private store 读取
...
PARENT_UID = <uid>（**本环境 FAIL，目标 <uid>**，见 §4）
CHILD_UID  = <uid>
```
...
`launchctl asuser/bootstrap user/<uid>` EPERM、无 setuid helper、无存储口令；
2026-08-08 uid505 cutover 也是当时 root 一次性完成的）。

1. `PARENT_UID = <uid> ≠ <uid>`：trusted Router parent 必须以 <svc-user>/<uid> 运行
   （LaunchDaemon `UserName=<svc-user>` 或等价一次性 bootstrap）。
2. `A_READ_CREDENTIAL_STORE = READABLE`：store 文件须 <uid>-owned 0600 落在
   <uid>-private 区（`~/.openclaw/credentials` 已实证对 <uid> EPERM）；<uid> 无
   chown 权限，当前 store 与 child 同 uid，OS 层无法区分（与
   OPENCLAW_TRUSTED_CREDENTIAL_STORE_AND_HOST_EXEC_V1 报告的
   「<uid> 可读边界」同一结论）。
3. 附属：<uid>→<uid> 的子进程降权需要 setuid-root spawn helper
   （`DSH_AGENT_SPAWN_HELPER` seam 已实现）；同样需要一次性 root 安装。

root 就位后（一次性）：把控制面以 <uid> 运行 + store chown <uid> + 安装
...
PARENT_UID = <uid>            （目标 <uid> —— 需一次性 root bootstrap）
CHILD_UID  = <uid>
CHILD_HAS_SECRET = NO
CHILD_HAS_TOKEN = NO
REAL_AUTH = PASS
