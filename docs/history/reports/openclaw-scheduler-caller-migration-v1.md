...
三个 launchd daemon 脚本本体（`~/.openclaw/…`，均以 <svc-user>/UID505 运行）的
...
- `~/.agent-core/scheduler/` 建立 + <svc-user> ACL（`~/.agent-core` search ACL +
  scheduler 目录全量 `file_inherit,directory_inherit`，镜像 `~/.openclaw/cron`
  既有授权模式）；
- forum-scheduler.sh 编辑后原样恢复文件级 <svc-user> ACL（原子替换会新建 inode）。
...
2. **存量状态**：forum-scheduler 的 state 文件里若有指向旧 OpenClaw jobId 的
   ...
   预期，无需人工处理。
3. **store 目录**：`~/.agent-core/scheduler/` 已建好且 <svc-user> 可写；当前为空，
