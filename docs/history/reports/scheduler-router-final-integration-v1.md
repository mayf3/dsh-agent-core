...
**真实发送证据**（announce job，`{mode:'announce', channel:'feishu', to:'chat:oc_<redacted>'}`，
即本仓库既有测试群）：

```
PASS  DELIVER_SEAM_WIRED           — job3 finished delivered=true deliveryStatus=delivered
PASS  DELIVER_EXISTING_OUTBOUND_SEAM — feishu.reply create -> chatId=oc_<redacted>… messageId=om_x100b68ca44cab8a0c2e58b9b11cd7c4 code=0
