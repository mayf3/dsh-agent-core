...
Authorize, only after acceptance and every prerequisite below, one bounded Agent Core
production release of the already-implemented canonical capability:
...
```
The release MUST preserve target-owned execution identity, trusted source derivation,
one new target Run/Turn per accepted send, bounded receipt/reply behavior, and no
automatic replay or ping-pong.
...
- production apply and rollback authority only after this exact Spec is accepted with
  `production_apply_authority: contracts` by an Owner-authorized lifecycle transaction.
...
- fleet-wide enablement, historical-session targeting, external-channel delivery, delegation,
  or automatic follow-up turns;
...
OBSERVED_GITHUB_MAIN = 495b163caea28c4c34b1aa65d131f93184e7f068
...
```
The accepted local parent owns the complete `agent_session_send` product contract but
explicitly grants no production apply. This new deployment Spec is therefore `NEW`, not
an amendment or successor. It does not alter the parent.

The external Auth parent supplies grammar and issuance architecture only. It does not
register or semantically authorize `agent-session-messaging`. A separate accepted
auth-service bounded child CCR (or a whole successor if auth-service review requires one),
its implementation, and its production deployment MUST precede any Grant or Agent Core
apply. This Spec cannot create that cross-repository authority.

## 4. Current State

- `STATE-DEP-001` — Subject: canonical Session Messaging product authority and source;
  revision: `github/main@495b163caea28c4c34b1aa65d131f93184e7f068`;
  environment: repository authority branch; observed at: `2026-09-03T00:36:26Z`;
  basis: `OBS-DEP-001`, `EVD-DEP-001`.
- `STATE-DEP-002` — Subject: Agent Core production release surface under
  ...
  below; environment: production host; observed at: `2026-09-03T00:36:26Z`;
...
- `STATE-DEP-004` — Subject: production deployment authority and release artifact for
  Session Messaging; revision: repository scan at `github/main@495b163...`;
  environment: Agent Core repository; observed at: `2026-09-03T00:36:26Z`;
  basis: `OBS-DEP-004`, `EVD-DEP-004`.
Production target root for every row is `/usr/local/libexec/agent-core/app/`.
`ABSENT` is a required preimage condition, not a wildcard.
...
- Source revision: `github/main@495b163caea28c4c34b1aa65d131f93184e7f068`
...
- Result: acceptance `d6c781696b1c...` and implementation commits `07102da0d241...`,
  `8258acb8e37d...` are ancestors of main; the accepted parent blob is `a2632f90...`.
...
- Result: 11 paths have the exact listed older blobs and 6 paths are absent.
...
- Source revision: candidate `7921f4a...` blob `cdd3eacf...`; main `495b163...`
  blob `c407b064...`
...
- Relation: SUPPORTS
- Bound coordinates: Agent Core `github/main@495b163...`, observed `2026-09-03T00:36:26Z`
...
- Relation: SUPPORTS
- Bound coordinates: Agent Core `github/main@495b163...`
...
- Target: `CLM-DEP-003`
- Relation: SUPPORTS
- Bound coordinates: candidate `7921f4a...`; main `495b163...`; compose blobs in OBS-DEP-005
...
- Decision: require accepted and deployed auth-service audience/scope authority before Grant supply.
...
- Decision owner: repository Owner
- Decision: stage and verify release plus rollback faces, apply all 17 files under a lock,
  restart once, then prove health; any mismatch or failure stops or rolls back the whole face.
...
Before Agent Core staging or Grant supply, auth-service MUST have separate accepted
authority and a deployed active machine-only agent audience with exact resource
`agent-session-messaging` and exact scope `agent.session.send`. Token issuance for an
authorized disposable source principal MUST be positively proved, and wrong resource,
wrong scope, human/delegated access, wildcard, and alias forms MUST fail closed. Grammar,
an implementation branch, a database Grant, or this Spec alone MUST NOT satisfy the Gate.

### CTR-DEP-003 — Immutable staging and preimage gate

The operator MUST create non-secret root-owned release and rollback artifacts before
apply. Every release hash MUST equal `CTR-DEP-001`; every live target MUST equal its exact
§4 preimage including required absence; rollback content MUST re-hash to those preimages.
Artifacts MUST contain no credential, token, Grant material, or environment dump. Any
unexpected path, blob, mode, owner, symlink, or missing rollback face MUST stop without
production mutation.
...
```text
A Auth audience/config Authority + implementation + production deployment + token proof
→ B this Agent Core production Authority + immutable artifact + preimage Gate + apply
→ C separately authorized minimal Grant supply
...
Only an authorized actor using an Owner-approved native privileged path MAY atomically
replace the 11 present files and create the 6 absent files in §4. It MUST preserve the
declared modes/ownership, fsync staged files/directories, restart the Agent Core parent
exactly once after complete apply, and verify a fresh parent plus fresh relevant child
generation and health. No unrelated file, config, data, launch definition, or service may change.

### CTR-DEP-006 — Minimal Grant supply

Only after `CTR-DEP-002` and `CTR-DEP-005` PASS, a separate accepted Grant-supply authority
and Owner gate MAY grant exact `(resource=agent-session-messaging,
scope=agent.session.send)` to the named disposable source Agent principal(s) required by
the canary. It MUST NOT grant the target, the fleet, a human/service principal, aliases,
wildcards, broader scopes, or new credentials. Grant readback and token claim verification
are mandatory; Grant work is not part of the 17-file artifact.

### CTR-DEP-007 — Fresh catalog/header and A2A proof

After a verified fresh runtime, a newly created ordinary source Session and a newly created
target-bound main Session/header MUST show `agent_session_send` exactly once with operation
`send`; `agent_wake`, `a2a_send`, `sessions_send`, and `send_message` MUST be absent as
messaging capabilities. One authorized source MUST send one nonce-marked message to a
different target Agent main Session. Evidence MUST bind request/correlation, source Agent,
target Agent, target `main`, receipt, one target-owned Run/Turn, terminal outcome, and any
bounded reply. The target MUST use target-owned identity/config/credential; source token,
Grant, credential, authorization header, or authority MUST NOT propagate. There MUST be
no replay, duplicate target Run/Turn, automatic reply-send, external-channel delivery, or
second canary attempt hidden under the same receipt.

### CTR-DEP-008 — Failure and rollback

Any failed preimage, integrity, Auth, restart, health, header, identity, delivery,
exactly-once, or leakage check MUST fail closed. If production files changed, the operator
MUST restore the complete 17-path §4 preimage face (deleting only the six paths whose
preimage is `ABSENT`), restart once, and re-prove baseline health and hashes. Evidence and
audit rows MAY remain; secrets MUST NOT be recorded. A Gate, dialog, artifact, restart, or
receipt alone MUST NOT be reported as deployment or canary success.

### CTR-GOV-001 — Lifecycle and final-head authority

This proposed revision authorizes no code, artifact apply, restart, Auth change, Grant, or
canary. After independent exact-head review and explicit Owner authority, one docs-only
lifecycle transaction MAY set `status: accepted`, `implementation_authority: contracts`,
and `production_apply_authority: contracts` with review provenance and no normative change.
An independent `FINAL_HEAD_RECHECK=PASS` on that exact accepted head and unchanged external
authority coordinates is required before merge. Merge of that exact head activates only
the contracts above; each external/production mutation still requires its named Gate.
...
- Method: rebuild manifests twice, compare hashes, inspect closure/preimages/modes/owners, dry-run apply
...
- Expected result: exact 17/17 release and rollback faces; zero extra paths; every Gate passes
- Failure condition: any mismatch, mutable source, missing rollback byte, secret, or extra path.

### ACC-DEP-002 — Auth prerequisite

- Contracts: `CTR-DEP-002`
- Method: verify accepted auth authority/deployed registry, then issue positive and negative tokens
- Environment: deployed auth-service and disposable agent client authorized by separate authority
- Required evidence: exact auth head/deployment, registry/audience readback, sanitized token claims,
  negative matrix and zero-Grant-before-registration proof
- Expected result: exact resource/scope succeeds only for the permitted agent profile; all negatives fail
- Failure condition: missing authority/deployment, issuance via alias/wildcard, or Grant used as a substitute.
...
- Required evidence: phase receipts, authorization identity, before/after manifests, restart receipt,
  fresh parent/child coordinates, health output, unrelated-path no-change proof
- Expected result: exactly one complete apply and restart; all 17 target hashes equal release face
- Failure condition: skipped/out-of-order phase, partial face, stale child, extra restart/change, or failed health.

### ACC-DEP-004 — Grant, header, and canary

- Contracts: `CTR-DEP-006`, `CTR-DEP-007`
- Method: apply the separately authorized minimal Grant, create fresh sessions, inspect source/target
  headers, execute one nonce-bound send, correlate runtime/audit/session/run evidence
- Environment: production Agent Core plus deployed Auth audience and disposable authorized source/target
- Required evidence: sanitized Grant/token readback, two fresh session/header IDs, capability inventories,
  one request/receipt/correlation chain, target-owned identity proof, terminal reply/timeout, replay/leak checks
- Expected result: canonical tool only; one source-to-target-main send creates exactly one target Run/Turn
  under target identity with no authority/credential propagation or replay
- Failure condition: alias visible/callable, missing or duplicate Run/Turn, source-owned target execution,
  disclosure/leakage, external delivery, automatic ping-pong, or inconclusive target-bound evidence.

### ACC-DEP-005 — Failure and rollback

- Contracts: `CTR-DEP-008`
- Method: inject a non-secret staging failure in rehearsal and verify rollback logic; on real failure,
  inspect complete rollback and baseline health
- Environment: isolated rehearsal; production only if a real authorized apply fails
- Required evidence: injected-failure transcript, rollback hashes, deletion list for six absent preimages,
  restart/health receipt, preserved audit evidence
- Expected result: zero partial face; complete exact preimage restoration and healthy baseline
- Failure condition: mixed face, deletion outside six declared paths, missing restart/health, or lost evidence.

### ACC-GOV-001 — Authority lifecycle

- Contracts: `CTR-GOV-001`
- Method: independent exact-head semantic review, Owner acceptance, lifecycle-only diff, final-head recheck
- Environment: isolated docs branch against current authority branch and pinned auth external head
- Required evidence: reviewer identity/verdict, zero blockers, Owner authority, lifecycle commit/diff,
  `FINAL_HEAD_RECHECK=PASS`, unchanged normative digest and external-authority recheck
- Expected result: accepted/contracts/contracts only after all governance evidence; no production mutation
- Failure condition: semantic drift, changed head after recheck, missing Owner authority, external drift,
  or any implementation/production action before merge.
...
There is no Session data migration and no credential migration. Existing Agent Core capability
behavior is preserved. The six newly created paths are additive only as one member of the exact
release face; partial presence is invalid. Rollback restores all eleven old blobs and deletes only
the six `ABSENT` preimages, then restarts once and proves baseline health. Auth registry and Grant
rollback belong to their separate accepted authorities and MUST NOT be improvised by this artifact.
