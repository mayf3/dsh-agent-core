...
amended_by: mayf3
amendment_ref: r2 = BOOT_CLOSURE_AMENDMENT (first apply crash: missing forum-moderation.js; §4.1); r3 = BOOT_GENERATION_AMENDMENT (second apply crash 23:23Z: ESM named-export coupling broker/index.js→forum.js normalManifests; §4.2)
...
### In scope
- the exact 28-file release closure and current production preimage census in §4 (r3: r2 18 rows + rows 19–28 added by §4.2);
...
| 19 | `packages/agent-provisioning/src/index.js` | `34479d812b14db870c568a1512a767a982220e31` | 24031 | `PRESENT` | — |
| 20 | `packages/agent-provisioning/src/plugin-artifact.js` | `89394ab32dcbc3bc0acf6b3e8468510b28f11add` | 2610 | `ABSENT` | — |
| 21 | `packages/agent-provisioning/src/shared-codex.js` | `963d03bea20110dc08c012dca975212cc27b1195` | 3585 | `ABSENT` | — |
| 22 | `packages/agent-router/src/route-chain.js` | `e2f69dac5bb1d18d287405b15738d29154780abc` | 22928 | `PRESENT` | — |
| 23 | `packages/broker/src/capabilities/forum.js` | `b64b73578f9645777025d8bff74e82f551de3866` | 18354 | `PRESENT` | — |
| 24 | `packages/broker/src/error-detail-sanitizer.js` | `25e02239255f95c55e59454bd32703550e3ad5b0` | 6318 | `ABSENT` | — |
| 25 | `packages/broker/src/mapping.js` | `74e83e11660a0ea5a9dffc47fe2a8897f17f9809` | 12867 | `PRESENT` | — |
| 26 | `packages/broker/src/registry.js` | `7cd71350226ebe63bada5bbfa1372fabd81585ee` | 8540 | `PRESENT` | — |
| 27 | `packages/broker/src/schema.js` | `1bd36bbe5f3fc796dffdf633b557fbb7fff2741b` | 21558 | `PRESENT` | — |
| 28 | `packages/broker/src/transport.js` | `942e9880ff11fbc672481f66f7e96d35778dfc4f` | 29266 | `PRESENT` | — |

## 5. Observations

### §4.2 AMENDMENT r3 — BOOT-GENERATION CLOSURE (NEW_EVIDENCE 2026-09-04)

第二次生产 apply（2026-09-03T23:23Z）再次 fail-closed：新父进程已起但随即 `SyntaxError: './capabilities/forum.js' does not provide an export named 'normalManifests'`（candidate `broker/src/index.js`@`eec8c6f9` 与 live `forum.js` 的 ESM 命名导出版本耦合）。r2 的存在性闭包不检查导出面。本 amendment 把闭包从「实现 PR 文件集」升级为 **candidate 启动世代闭包**：

- **闭包定义（r3）** = `7921f4a` 树内从 `scripts/production-runtime.mjs` 出发的入口 BFS（101 模块）中与 live blob 不一致的文件，**减去受保护 live 集**：`capabilities/workflow.js`（Lane A `db7688fe`——candidate `83df50eb` 是旧 transition 面，部署即回归）、`feishu-connector/src/index.js`、`scheduler-router/src/index.js`、`model-overrides.js`（combined-deploy v2 excluded drift pins，有意生产漂移）。共 **28 文件** = 25 个 daemon boot-closure 文件 + 3 个原审计闭包中非 daemon 可达文件（demo-server index/session-seam、shared-codex-migration-executable——独立入口/CLI 伴侣，保持树一致性）（r2 的 18 全保留 + rows 19–28）。
- **导出面 Gate（强制）**：闭包文件对每个保留 live 目标的具名导入 ⊆ 该 live 文件导出面（r2 教训一般化）。机械验证 EXPORT_INCOMPAT=0（`broker/index.js → workflow.js` 仅需 `manifests` ✓；candidate `registry.js` 支持多 operation manifests ✓）。
- **真实 boot rehearsal（强制）**：Owner 执行前必须沙箱真实 boot（生产 node 二进制 + live 副本 + 25 文件 + 合成 root state + 独立端口 + 无代理 env）至 health ok 存活 ≥60s。已执行：`production runtime ready` + health ok；`DEFAULT_MANIFESTS` 含 `agent_session_send` ✓ `workflow_execute` ✓ `workflow_transition` ABSENT ✓。
- **Surface 披露**：candidate `broker/src/index.js` 同时注册 forum normal pack（12 manifests）；实际使用仍由 per-agent Grant 门控。moderator 面默认不注册。
- 其余合同不变；回滚面以 ROLLBACK_MANIFEST 为准。
...
### DEC-DEP-001 — Pin the audited 28-file candidate boot-closure release (r3)
...
- Decision owner: repository Owner
- Decision: stage and verify release plus rollback faces, apply all 28 regular `root:wheel` `0644` files under a lock, restart once, then prove health; any mismatch or failure stops or rolls back the whole face.
...
An authorized builder MUST extract exactly the 28 §4 blobs from
...
Any failed preimage, integrity, Auth, restart, health, header, identity, delivery, exactly-once, or leakage check MUST fail closed. If files changed, the operator MUST restore the complete 28-path §4 preimage face and metadata (deleting only the `ABSENT` preimages per the rollback manifest), restart once, and re-prove baseline health/hashes. If Phase C applied a Grant, D/E failure or terminal completion MUST trigger only the separately authorized compensation in `CTR-DEP-006` and prove absence. Evidence/audit rows MAY remain; secrets MUST NOT be recorded. A Gate, dialog, artifact, restart, or receipt alone MUST NOT be reported as success.
...
Before that commit, the authorized actor MUST fetch current main, bind its exact head, prove the accepted parents/external authority have not drifted, and recheck the 28 release paths against their §4/§4.2 provenance and all 28 production preimages/metadata. After the lifecycle commit, an independent reviewer MUST verify every changed byte is allowlisted; all non-allowlisted normative bytes are identical; new values match Owner authority and review identity/outcome; `SEMANTIC_DELTA=NONE` means product/deployment behavior is unchanged; and the closure-path checks remain PASS. `FINAL_HEAD_RECHECK=PASS` binds that exact accepted head; the head MUST NOT change before merge. Merge activates only these contracts.
...
- Expected result: exact 28/28 regular `root:wheel` `0644` release face and exact rollback metadata/absence face; zero extra paths
...
- Expected result: exactly one complete apply/restart; all 28 targets equal release blobs and are regular `root:wheel` `0644`
