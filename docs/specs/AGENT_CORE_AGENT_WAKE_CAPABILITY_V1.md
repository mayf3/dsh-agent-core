...
date: 2026-08-31
revision: r2（2026-08-31 审计 REVISE：关闭三个 blocker，全部为代码事实
...
> 状态：**proposed**（r2 — 审计 REVISE 的三个 blocker 已关闭：relay local
> 支持、localHandlerResolver 注入路径、Router 错误码映射事实；不变量保持
...
> NEXT_TASK = 唤醒 审计（对 r2 的 independent re-review）。
...
```text
args（严格校验；未声明参数 / 违反约束 → invalid_arguments fail-fast，
     handler 零执行，fake-router 可断言零调用）:
...
child 侧 mapping.js 既有 fail-closed 收敛，不降级不吞错）:
  invalid_arguments        # child 侧 args 校验（validateArgumentsDetailed，
                           # R2 全部约束）；relay 传输通道死亡（relay.js:76）；
                           # 控制面无 brokerGateway（agent-router index.js:595）
...
MOD   packages/production-runtime/src/compose.js        ← blocker #2 + 注入
        ...
        — handler 冻结形态：R2a/R4 派生（message 模板、requestId 公式）
...
ACC-001  agentWakeManifest 过 validateManifest（local/http 互斥、错误表
         ...
         manifest 字节不变
ACC-002  args 校验 fail-fast：缺任一必填 / 未声明多余键 / targetAgentId
         非 ^agt_[a-z0-9-]+$ / workflowInstanceId 含 ':' 或违 charset /
         reason 空或 >240 或含控制字符 → invalid_arguments 且 fake
         router 零调用
...
PRODUCT_CODE_CHANGE      = NONE（零 packages/ 文件改动；仅本 Spec 文件
                           就地修订 r1 → r2）
SPEC_REVISION_ROUND      = r2（audit REVISE 三 blocker 关闭；取证 =
                           只读 packages/ 源码 + git diff，零写入；r1 语义
                           裁定零回退）
HR_DISPATCHER_MODIFIED   = NO（r4 同步 AMEND = 其自身评审轮，G6）
SPEC_PR                  = NOT_OPENED（本轮任务未授权开 PR；当前分支含
                           无关 WIP，PR 开立归 operator 裁量）
...
TASK_TYPE = SPEC_AUTHORING_ONLY
SPEC_ID = AGENT_CORE_AGENT_WAKE_CAPABILITY_V1
SPEC_STATUS = proposed（r2 — 审计 REVISE 三 blocker 关闭：relay local
  支持 / localHandlerResolver 注入路径 / Router 错误码映射事实）
...
DELIVERED_SECTIONS = PROPOSED_SPEC（§0–§3,§7,§8）+ IMPLEMENTATION_CLOSURE
  （§4，r2 重写）+ CANARY_PLAN（§5）+ ACCEPTANCE_CRITERIA（§6，ACC-010/
  011 新增）
