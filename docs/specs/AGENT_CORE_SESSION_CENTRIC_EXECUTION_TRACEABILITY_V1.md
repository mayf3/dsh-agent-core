...
accepted_reviewed_head: 1be36f37fa7b67b4849a8aae97aa2c30a185bedb (FINAL_SEMANTIC_AUTHORITY_HEAD — full fresh authority re-audit ACCEPT / SHIP_BLOCKERS=NONE over SC-1/SC-2, CTR-SCT-001..008 incl. all round-3/4 amendments, T1-T9 incl. the T8-A/B/C frozen cases; Owner acceptance bound to this exact head)
...
- F-SCT-5：postdeploy canary invoker 身份标记，使 canary occurrence 的 `sessionCreated`
  ...
  侧已诚实降级）。
- F-SCT-6：scheduler-history writer（history lateSettlement/run_terminal 事件与 RunRecord）持久
  late pre-start discriminator（如经既有 error_code 字段承载 terminalEvidence 分类），使
  occurrence-rotation/history-only 场景在 ledger 丢失后仍可判 `not_created`；需独立 scope 授权与
  分类传播评审。当前该场景的诚实上限 = `sessionCreated=unknown`（designated id 带 unknown
  标注，不冒充已存在，SC-1 不被违反）。
