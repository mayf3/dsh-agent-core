...
amendments: []  # V4 whole-successor: V3's accepted amendments (AMENDMENT_1/2/3) are carried
                ...
                # the additive job_disposition authority from
                # SCHEDULER_SELF_HEALING_FROM_FEISHU_V1 §3 (C-SH-003) by reference.
owners:
  - mayf3
...
> **PROPOSED whole-Spec successor**（2026-09-19 candidate；base = V3 accepted
> `23b2332f9c3b4a35511c63dec367f2e5d97c0bdc`）。本文件逐字携带 V3 全部正文与已纳入的 Amendments
> 内容；唯一 semantic delta = §V4-DELTA：`self_ops` 三动作
> `status|reconcile_turn|job_disposition`、LOCAL、**model-visible**（V3 的
> `infrastructure=true` 对 self_ops SUPERSEDED——Owner ruling SELF_OPS_MODEL_VISIBLE=YES,
> 2026-09-19）。registry 通用 infrastructure 过滤不变；`agent_session_send_reconcile` 维持
> infrastructure=true（hidden）。job_disposition 的结果合同 =
> SCHEDULER_SELF_HEALING_FROM_FEISHU_V1 §3（C-SH-003，按引用纳入）。
> Authority mapping：C-SH-003 / Phase-D model-visible requirement is satisfied through
> Tools V4（Phase-D = SCHEDULER_SELF_HEALING_FROM_FEISHU_V1 §0/§3 的真实飞书验收阶段；
> 本引用依据 Owner ruling SELF_OPS_MODEL_VISIBLE=YES, 2026-09-19）。Acceptance 仅激活
> contracts；production apply authority 仍为 none。

## V3. Normative reading rule

本文件自包含：V3 全部正文与 Amendments 在 V4 中逐字携带并继续具有 normative force，除非被
§V3.1 或 §V4-DELTA 逐项列为 `REPLACED`/`ADDED`。旧正文中的 V1→V2 acceptance、旧 commit/date、candidate/current-state、
“V2”自称与 Final Output 是 historical provenance，不是 V4 lifecycle truth。不得用此规则隐式替换
...
## V4-DELTA — The only semantic delta of this whole-successor

Owner ruling（2026-09-19，SELF_OPS_MODEL_VISIBLE=YES）：V3 中 `self_ops` 的
`infrastructure=true` 对 self_ops **SUPERSEDED**。本节逐项 REPLACED/ADDED；除本节外零语义变化。

```text
REPLACED =
  §V3.1 manifest 行 `self_ops   # new LOCAL, infrastructure=true, selector=action`
    (replaced by: `self_ops   # LOCAL, model-visible, selector=action; actions exactly
     status|reconcile_turn|job_disposition` — V4 起 self_ops 移除 infrastructure 标记，
     经 registry 通用过滤正常注册为 model-visible；registry 过滤本身零改动)
  CTR-V3-TOOL-001 schema json 块（两动作）
    (replaced by: 三动作 json 块——新增 {"action":"job_disposition","job_id":"..."}；
     job_disposition 的结果合同 = SCHEDULER_SELF_HEALING_FROM_FEISHU_V1 §3（C-SH-003）
     v2 additive 字段集，按引用纳入；closed schema 不变：必填且仅 job_id，unknown
     fields 在任何 read 前 invalid_arguments)
  CTR-V3-AVAIL-001 "Broker readiness 扩展 self_ops.status/reconcile_turn booleans"
    (replaced by: readiness 扩展 self_ops.status/reconcile_turn/job_disposition 三个
     booleans；缺 provider/store/Router seam/trusted caller 时 capability_unavailable 不变)
  V3.2 "Current tool authority until V3 acceptance = ..._V2"
    (replaced by: Current tool authority until V4 acceptance =
     AGENT_CORE_SELF_SERVICE_SCHEDULER_TOOLS_V3)
  V3.2 "actions exactly `status|reconcile_turn`"
    (replaced by: actions exactly `status|reconcile_turn|job_disposition`)
  §1 Goal 动作行 self_ops(action=status) / self_ops(action=reconcile_turn, ...)
    (replaced by: 增加 self_ops(action=job_disposition, job_id))
  V3.5 `SPEC_ID = ..._V3` / `STATUS = accepted` /
       `REPLACES_ON_ACCEPTANCE = ..._V2` / `NEW_TOOL = self_ops(status,reconcile_turn,job_disposition)` /
       `INDEPENDENT_SPEC_REVIEW = PASS_AT_23b2332...`
    (replaced by: SPEC_ID = AGENT_CORE_SELF_SERVICE_SCHEDULER_TOOLS_V4;
     STATUS = proposed（acceptance 时置 accepted）; REPLACES_ON_ACCEPTANCE =
     AGENT_CORE_SELF_SERVICE_SCHEDULER_TOOLS_V3;
     NEW_TOOL = self_ops(status,reconcile_turn,job_disposition);
     INDEPENDENT_SPEC_REVIEW = 记录于本轮 exact-head review record)
     V3.5 `IMPLEMENTATION_ALLOWED_NOW = AFTER_ACCEPTED_V3_MERGES_TO_MAIN`
       (replaced by: IMPLEMENTATION_ALLOWED_NOW = AFTER_ACCEPTED_V4_MERGES_TO_MAIN)

ADDED =
  Authority mapping：C-SH-003 / Phase-D（SCHEDULER_SELF_HEALING_FROM_FEISHU_V1 §0/§3
  真实飞书验收阶段）model-visible requirement is satisfied through Tools V4（依据 Owner
  ruling SELF_OPS_MODEL_VISIBLE=YES, 2026-09-19）。
  真实工具清单 regression（验收硬项，不得以 manifest/readiness/handler 存在性测试替代）：
    MODEL_TOOL_LIST_CONTAINS_SELF_OPS=YES（actions 恰为三者）
    MODEL_TOOL_LIST_CONTAINS_AGENT_SESSION_RECONCILE=NO
    MODEL_TOOL_LIST_CONTAINS_SCHEDULER=YES（unchanged）
  Supersession：V4 accepted → V3 superseded；reciprocal backlinks（V3 frontmatter
  superseded_by = V4，另加 scoped 注记 V3 自身的 model-visible 声明由 V4 落地）。
  PRESERVED（零改动清单）：scheduler manifest 与 7-action union；registry 通用
  infrastructure 过滤；agent_session_send_reconcile 的 infrastructure=true（hidden）；
  trusted caller identity（Parent Runtime 派生）；self-only authorization；zero Auth
  minting；closed schemas；termination-only reconcile；foreign access forbidden；
  raw-store forbidden；blind retry forbidden；receipt/idempotency。
```
...
Current tool authority until V4 acceptance = AGENT_CORE_SELF_SERVICE_SCHEDULER_TOOLS_V3
...
exactly `status|reconcile_turn|job_disposition`（V4-DELTA）。Parent trusted caller 是唯一 subject；self path zero Auth calls。
...
Model-visible tools exactly 增加 `self_ops`（V4-DELTA：含 job_disposition、model-visible、
无 infrastructure 标记）；禁止注册 per-action aliases 或 `scheduler_reconcile`：

```text
scheduler  # inherited unchanged
self_ops   # LOCAL, model-visible, selector=action; actions exactly status|reconcile_turn|job_disposition
```
...
{"action":"job_disposition","job_id":"..."}
```

`job_disposition` 的结果合同 = SCHEDULER_SELF_HEALING_FROM_FEISHU_V1 §3（C-SH-003）v2
additive 字段集，按引用纳入（SELF_HEALING 仍是 disposition semantics 的 authority）。
...
Broker readiness 扩展 self_ops.status/reconcile_turn/job_disposition 三个 booleans（V4-DELTA）。缺 Runtime provider、Scheduler V3 store、
...
```text
SPEC_ID = AGENT_CORE_SELF_SERVICE_SCHEDULER_TOOLS_V4
STATUS = proposed（acceptance 时置 accepted）
REPLACES_ON_ACCEPTANCE = AGENT_CORE_SELF_SERVICE_SCHEDULER_TOOLS_V3
...
PRODUCTION_APPLY_AUTHORITY = none
IMPLEMENTATION_ALLOWED_NOW = AFTER_ACCEPTED_V4_MERGES_TO_MAIN
INDEPENDENT_SPEC_REVIEW = PENDING（本轮 exact-head review record 为准）
```
...
self_ops(action=job_disposition, job_id)  # V4-DELTA; §3 v2 contract by reference
```
