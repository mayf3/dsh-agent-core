...
name→id mapping point; Router `switchAgent`/`deliver` (hence
`agent_session_send`) consume it. It (a) raises `AGENT_NOT_FOUND` for a
DISABLED agent, (b) returns the FIRST name match via `Array.find` — a legal
config may contain duplicate display names because `normalizeDefinition`
rejects duplicate ids only — so a silent wrong-target pick is representable
today, and (c) has no machine-readable ambiguity outcome.
...
- `resolve { query: string }` — one exact reference. Classification, in
  ...
  (2) otherwise exact display-name equality after `trim()` + lowercase
  fold. Exactly one of:
  ...
  - `{ status: 'ambiguous', query, candidates: [{ agentId, name, enabled }, ...] }`
    ...
    nothing else; the tool NEVER selects one and NEVER guesses.
...
- `list {}` — `{ agents: [{ agentId, name, description, enabled }, ...] }`
  in config order. This is the discoverability half of the same primitive;
  it reads the same snapshot as `agent.definition.read list` and that
  sibling capability remains unchanged.
...
closed error table is `invalid_arguments` and `unsupported_operation` plus
...
PER_CONSUMER_GRANT_DECISION_REQUIRED=NO). This creates NO new auth-service
...
classification) and classifies it: exact id match wins over name match
(mirroring `resolveAgentRef` precedence); a DISABLED Agent resolves
explicitly as `{ status: 'resolved', agent: { ..., enabled: false } }` —
existence truth per CTR-IAD-003, never silently `not_found`; duplicate ids
...
| ACC-ADT-002 | 001, 003 | semantic matrix T1–T8 against a temp Agent Definition config (exact enabled id; exact disabled id; exact name case-insensitive; trimmed name; duplicate names; unknown name; unknown id-shaped string; whitespace-only query) | resolved/enabled discrimination exact; ambiguity explicit with full candidate set; not_found explicit; invalid input fails closed; id-over-name precedence proven |
