...
| `AGT_CTO_AGENT_ORDERED_ROUTE_CHAIN_IMPL_V2` | proposed / non-authoritative replacement candidate | contracts only after coordinated three-V2 acceptance + merge; currently none effective | complete standalone IMPL V1 successor: preserves loader/executor/reuse/deadline/journal/Scheduler contracts and adds admission-first quota correction plus a closed one-shot target canary seam |
...
| `AGT_CTO_AGENT_ORDERED_ROUTE_CHAIN_V2` | proposed / non-authoritative replacement candidate | none | complete standalone Parent V1 successor: preserves ordered-chain policy and freezes existing OAuth/plugin reuse, Home 0755, and GLM quota post-admission STOP |
