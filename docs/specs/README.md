...
| `PRODUCT_API_VOICE_TRANSCRIPTION_V1` | proposed (2026-09-08) | contracts | Mobile Presence voice session 的同一 Product API 私有转写路由：POST /v1/voice/transcription（audio/wav 有界 utterance → {"text"}）、512KB/15s 上限、确定性 400/413/415/503/504、自托管 ASR 执行（Paraformer-zh via sherpa-onnx 主选）、raw-audio 零持久化 + 日志白名单；history-listener 复用禁止；/v1/message 语义不动；sibling = agent-core-mobile MOBILE_AGENT_PRESENCE_RUNTIME_V1（PR #17，client seam） | none |
