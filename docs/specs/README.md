...
| Spec | Current lifecycle | Implementation authority | Authority role |
|---|---|---|---|
| `AGENT_CORE_FLEET_SHARED_CODEX_AUTH_V1` | proposed / non-authoritative | contracts after independent review, Owner acceptance, atomic whole-authority supersession, and merge | proposed complete standalone successor of the Parent/Impl/Activation V2 authority set: preserves ordered-route safety while replacing per-home/no-refresh Luna credentials with one Permission-Model-A canonical store, serialized refresh intent, fail-closed uncertain outcome, and one canonical Owner reauth |
