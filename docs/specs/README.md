...
| `AGENT_CORE_AGENT_SESSION_MESSAGING_DEPLOYMENT_V2` | proposed | none | complete standalone V1 successor proposing one declared transaction-order correction: B1 exact file-face apply/fresh parent → separately authorized temporary Grant → D/B2 ordinary Session/header plus fresh relevant Router-owned child → bounded canary; no production authority before exact-head review, Owner acceptance, lifecycle finalization, and merge |
| `AGENT_CORE_SCHEDULER_RUNTIME_DEPLOYMENT_V1` | proposed | contracts | inactive exact `18f96e2` ten-file Scheduler deployment after ASM/Auth/Grant prerequisites: post-ASM compose guard, native Owner transaction, one restart/equal-face rollback, then accepted 15-minute hot-reload Feishu canary with whole-host legacy-path zero-access proof |
