...
| Spec | Current lifecycle | Implementation authority | Authority role |
|---|---|---|---|
| `AGT_CTO_AGENT_ORDERED_ROUTE_CHAIN_ACTIVATION_V2` | proposed / non-authoritative replacement candidate | contracts only after coordinated three-V2 acceptance + merge; currently none effective | complete standalone whole-authority replacement candidate for Activation V1: GLM primary with exact terminal-quota Luna backup, existing Luna reuse, Harness identity, audited IMPL V2 classifier, and controlled candidate/final canaries; activates only after Parent/IMPL/Activation V2 atomic acceptance and merge |
| `AGT_CTO_AGENT_ORDERED_ROUTE_CHAIN_ACTIVATION_V1` | accepted / current（Amendment 1 accepted；PR #94 merge `df3b299` 已进入 main） | contracts / contracts（gated） | sole active activation authority：Amendment 1 current meaning = Stage 1 GLM strict 已上线（builtin/zai/glm-5.3，primary=glm53，fallbacks=[]）+ Stage 2 Luna cold fallback deferred；Phase A routeKind/canonical identity/source-stamp capability已完成；全部 route-chain 安全 policy、credential、deployment、canary 与 isolation 边界保持；在 V2 原子 acceptance+merge 前不授权 Luna activation |
| `AGT_CTO_AGENT_ORDERED_ROUTE_CHAIN_IMPL_V2` | proposed / non-authoritative replacement candidate | contracts only after coordinated three-V2 acceptance + merge; currently none effective | complete standalone IMPL V1 successor: preserves loader/executor/reuse/deadline/journal/Scheduler contracts and adds the exact terminal pre-generation quota class, unsafe-429 STOP precedence, and a closed controlled-429 canary seam |
...
| `AGT_CTO_AGENT_ORDERED_ROUTE_CHAIN_V2` | proposed / non-authoritative replacement candidate | none | complete standalone Parent V1 successor: preserves ordered-chain policy and freezes existing OAuth/plugin reuse, Home 0755, exact `provider_quota_rejected_before_generation` hop, and ambiguous/unsafe 429 STOP |
...
Planned lineage is not current supersession: proposed Parent/IMPL/Activation V2 each keep
`supersedes: []` and all three V1 keep `superseded_by: null`. The three successor pairs must be
reviewed and accepted in one atomic transaction. Until that complete transaction merges main,
the three V1 remain the only active authorities; no V1/V2 mixed effect, implementation or
