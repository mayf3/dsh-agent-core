...
| Spec | Current lifecycle | Implementation authority | Authority role |
|---|---|---|---|
| `AGENT_CORE_SCHEDULER_PRODUCTION_RECONCILIATION_V1` | proposed (PR #164) | contracts（adoption/continued-operation/maintenance only；production_apply_authority=none） | 采纳并治理已在生产的 scheduler 10-file runtime face（generation 18f96e2）+ auth 1.7 scheduler face + scheduler.admin operational grant；§1 诚实记录 E1–E5 全部 EARLY_PRODUCTION_EXECUTION；THIS_AUTHORITY_DOES_NOT_RETROACTIVELY_AUTHORIZE_PAST_EXECUTION；supersedes stale-proposed `AGENT_CORE_SCHEDULER_RUNTIME_DEPLOYMENT_V1`（PR #159）；companion = auth-service `AUTH_SERVICE_SCHEDULER_PRODUCTION_RECONCILIATION_V1`（PR #52） |
| `AGENT_CORE_SCHEDULER_RUNTIME_DEPLOYMENT_V1`（PR #159，未合并） | stale proposed candidate（superseded as authority by the reconciliation spec；retained as historical input） | n/a | 历史工程输入：§3 十文件闭包与分析被 reconciliation spec 引用为事实记录；其冻结时序前置与实际执行路径已漂移 |
