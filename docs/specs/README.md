...
| `AGENT_CORE_SCHEDULER_RUNTIME_DEPLOYMENT_V1` | proposed | contracts | inactive exact `18f96e2` ten-file Scheduler deployment after canonical ASM/Auth prerequisites: sealed native Owner transaction, bounded quiescence/reconcile/rollback, then Phase-C Grant and exactly one cross-Agent fresh-session canary with no privilege propagation |
