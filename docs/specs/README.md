...
| `AGENT_CORE_AGENT_SESSION_MESSAGING_DEPLOYMENT_V1_CHILD_GENERATION_AMENDMENT` | proposed | none | narrow docs-only child amendment reconciling CTR-DEP-005 fresh-child proof with the accepted B→C→D order by splitting B1 file-face apply from B2 Stage-D child-generation closure; no production authority before exact-head review, Owner acceptance, and merge |
