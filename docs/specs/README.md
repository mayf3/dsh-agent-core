...
| Spec | Current lifecycle | Implementation authority | Authority role |
|---|---|---|---|
| `AGENT_CORE_SELF_SERVICE_SCHEDULER_TOOLS_V4` | accepted 2026-09-19; whole-successor supersedes V3; absorbs the SCHEDULER_SELF_HEALING_FROM_FEISHU_V1 §3 job_disposition authority by reference | contracts | `self_ops(status\|reconcile_turn\|job_disposition)` LOCAL + model-visible (infrastructure flag superseded for self_ops by Owner ruling SELF_OPS_MODEL_VISIBLE=YES, 2026-09-19); registry generic filter, scheduler manifest and all safety contracts unchanged; production apply authority remains none |
| `AGENT_CORE_SELF_SERVICE_SCHEDULER_TOOLS_V3` | superseded by V4; historical Amendments retained (carried as incorporated body) | historical | prior self-service Scheduler + self_ops authority incorporated by V4 |
...
| `AGENT_CORE_AGENT_PRINCIPAL_REVERSE_RESOLUTION_V1` | accepted (reviewed head `da38d7d`; round-1 ACCEPT/0 blockers + round-2 value-domain hardening re-confirmed ACCEPT; Owner exact-head acceptance 2026-09-21 via GOAL `AGENT_PRINCIPAL_REVERSE_LOOKUP_DELIVERY_V1` `APPROVE_NARROW_REVERSE_READ_PATH`) | contracts（CTR-APR-001..005 八文件闭包；生产 fleet runtime 部署另受 controlled operation 约束） | 反向只读 LOCAL capability `agent_resolve_principal_by_agent`：exact agentId（`^agt_[a-z0-9-]+$` 5..128）→ 经 Auth identity-directory 反向路由（`GET /api/v1/directory/agents/:agent_id/principal`，audience `identity-directory` × `auth.directory.read`）解析 exact stored Agent Principal UUID；trusted seam 镜像正向 CTR-EPAR 设计（gateway-frozen caller、acquireCallerToken、fixed origin、单次有界读、agentId 字节等价复核、本地 Definition deliverability 组合、disabled→`principal_disabled`）；closed 成功 `{agentId, principalId}` + 14-code 错误族；零 canonical lifecycle 声明、零新 registry/scope/audience；上游 pin = auth `AUTH_SERVICE_IDENTITY_DIRECTORY_REVERSE_RESOLUTION_V1` @ cf9b474（accepted，auth main 862eab3）+ `AUTH_SERVICE_INTERNAL_IDENTITY_DIRECTORY_V1` @ f2b7d4c |
