...
| Spec | Current lifecycle | Implementation authority | Authority role |
|---|---|---|---|
| `SCHEDULER_WATCHDOG_ROUTING_AND_STUCK_OCCURRENCE_RECOVERY_V1` | proposed (Owner design direction approved 2026-09-14; semantic + safety reviews PASS at `63d59b5…`, blockers none; final candidate remains Owner-gated) | none; candidate planning only; production deploy/store/fence/current-six mutation forbidden | proposed whole-Spec successor for same-Job `QUARANTINED_UNKNOWN`, four-result evidence reconciliation, root-cause incident dedupe with no unchanged reminder, disjoint business/failure/control-plane routing, and canonical enabled-Job health/readback |
