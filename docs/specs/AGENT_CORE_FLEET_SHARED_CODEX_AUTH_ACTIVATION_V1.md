...
scope:
  ...
  - quiesce/fencing, fresh-production gate, canary, small-batch and 91/91 rollout/rollback plans
owners:
...
canary → small batch → 91/91 的判别阶梯完成 fleet rollout——全部在 fail-closed、可精确回滚、
...
ACTIVATION_END_STATE =
  ...
  AGENTS_CANONICAL = 91/91
  PER_HOME_RUNTIME_OAUTH_USE = 0
  closure files = exact PR #127 blobs
...
- §4 冻结的 8-file selective per-file deployment closure（全部 blob 取自 PR #127 merge tree `1fdf8c3`）；
...
- quiesce/fence、fresh-production gate、canonical canaries（CEO/HR/Podcast/Shopping）、
  small batch（5-agent 判别集）、91/91 rollout 与各阶段 rollback 计划。
...
- 91 份 per-home OAuth 文件的修改或删除（保持只读 forensic evidence）；
...
## 4. Current State（fresh 2026-09-03；activation 前 Gate 必须 fresh 重读）

```text
INSTALLED_APP = /usr/local/libexec/agent-core/app（Luna-only breakglass 基线）
CLOSURE_PREIMAGES = model-overrides ea44819a / compose ==589194b / provisioning index b1d7b94d /
  route-chain 010df799；shared-codex*.js、plugin-artifact.js、migration-cli/executable ABSENT
CONFIG = agent-model-overrides.json v2，91/91 primary=luna fallbacks=[]，catalog=[luna]
CANONICAL_DOMAIN = ABSENT；PER_HOME_OAUTH = 91（roster 双射）；BYTE_EQUALITY_91 = YES（readiness
  evidence，非 production-time truth——activation Gate 必须 fresh 重证）
MEMORY_JS = 99d59bde…（known-good production blob；gate-only）
RUNTIME = launchd ai.agent-core.runtime，health PASS（4001/8790/8989）
```
...
生产代码闭包 = 恰好 8 个文件、8 个 blob，全部取 `1fdf8c3` 树；逐 blob sha/oid 在 artifact
manifest 冻结。禁止装入 main 上被其他 Goal 修改的变体（R1：compose.js main 版 +140 行污染、
migration-executable main 版 import 被重构）。`package.json`、tests、broker、memory.js、
任何 profile 业务文件不在闭包内。
...
v2→v3 升级 = `version:3` + routeCatalog/overrides 语义零变化（现有 luna 条目键集已是 v3 合法
形状）；`credentialFile` 注入由 runner `switchFleetConfig` 在 canonical commit 与 Model A
探针之后执行。preimage/rollback 同时覆盖 config 文件。

### CTR-ACT-004 — Quiesce and durable fence（对 LIVE Luna-enabled 基线）

`LUNA_DISPATCH_QUIESCED` / `REFRESH_WRITERS_QUIESCED` 必须对当前 91/91 Luna-strict LIVE 基线
...
gate script（transaction 脚本，非 product code）机械产出十门 receipt：roster=91（authoritative
registry，禁用 homes/ 目录列表充数）、91/91 byte equality、shape/account 一致、canonical intent
...
`installPinnedArtifact` 安装 dsh-codex `0.2.3` @ `75d98d5b`、sha256 `2d29f95f…` 到 91 profile
...
`verifyZeroPerHomeRuntimeOpens` 通过（CTR-SCA-005）：91 份 per-home 文件此后 read-only
forensic；任何 runtime open/refresh/fallback ⇒ fail closed 并按共享 auth 故障面处置。

### CTR-ACT-010 — Canary → small batch → 91/91 判别阶梯

禁止 0/91 → 91/91。顺序与判定字段：
...
3. **91/91**：批次展开 + `NEW_AGENT_DEFAULT` 与 provisioning 默认检查（防半迁移态）。
...
activation 前 gate script fresh-read 八项：`FLEET_ROSTER_COUNT=91`、`PER_HOME_OAUTH_COUNT=91`、
`ROSTER_BIJECTION=PASS`、`BYTE_EQUALITY_91=YES`、`CANONICAL_CREDENTIAL_PRESTATE=expected`、
...
- ACC-ACT-003：gate script 对 fixture 集合（91 等值 / 90+1 / fence 漂移 / pending intent /
...
CLOSURE_FILES = 8（PR#127 tree 1fdf8c3）
