...
| `ACC-CPM-008` | `CTR-CPM-004`, `CTR-CPM-007` | Scenario H: successful probe and install with bootstrap/network sentinels | isolated offline-store stage | install completes only from local content; sentinels prove no package-manager network/bootstrap | bootstrap, remote fetch, or unfrozen install occurs |
...
| `ACC-CPM-010` | `CTR-CPM-008` | Scenario J: inject failure at every declaration/discovery/probe/install/freeze boundary | isolated installer fixture with mutation sentinels | no backup exists and every production sentinel remains unchanged | backup or any production/launchd/active-root write occurs |
