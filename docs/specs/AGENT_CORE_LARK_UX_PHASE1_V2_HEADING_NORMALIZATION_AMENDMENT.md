...
IMPLEMENTATION_PERMISSION = EFFECTIVE_ONLY_WHEN_THIS_EXACT_ACCEPTED_ARTIFACT_ENTERS_MAIN
...
### 16.1 Acceptance Finalize Record

```text
ACCEPTANCE_FINALIZE = AGENT_CORE_LARK_UX_PHASE1_V2_HEADING_NORMALIZATION_AMENDMENT_ACCEPTANCE_FINALIZE
ACCEPTED_REVIEWED_HEAD = 0abe9968c0564a669c7f2ca6571a202b3edcaaf2
REVIEW_BASE = d506f81105e8aa05177a01b817ebe11dcc076ba5
REVIEWER_IDENTITY = ZCODE_UX_AUDIT_HEADING_NORMALIZATION_CURRENT_HEAD_2026_08_22
REVIEW_VERDICT = PASS
BLOCKERS = NONE
SEMANTIC_REVIEW_COMPLETE = YES
SEMANTIC_CHANGE_AFTER_REVIEW = NONE
ACCEPTED_AT = 2026-08-22T05:18:37Z
FINAL_ACCEPTED_HEAD = 2f66512988802a0e6f503dd0f3a44f24069b5d28
PRODUCT_SEMANTIC_CHANGE_AFTER_REVIEW = NONE
UNRESOLVED_AUTHORITY_CONFLICT = NONE
OWNER_RULING = ACCEPT_SDK_NATIVE_HEADING_NORMALIZATION (unchanged)
H1_H6_DISTINCT_RENDERING = NOT_REQUIRED (unchanged)
HEADING_TEXT_PRESERVATION = REQUIRED (unchanged)
HEADING_ORDER_PRESERVATION = REQUIRED (unchanged)
NATIVE_HEADING_TREATMENT = REQUIRED (unchanged)
ALL_HEADINGS_PLAIN_BODY_TEXT = FORBIDDEN (unchanged)
OTHER_UX_CONTRACT_CHANGE = NONE (unchanged)
PRODUCT_SEMANTIC_DELTA = HEADING_VISUAL_DISTINCTNESS_ONLY (unchanged)
PARENT_SPEC = AGENT_CORE_LARK_UX_PHASE1_V2 (accepted, blob 91513d140bfeb2747326a465bdc01d72c899c864, unchanged)
```

本 finalize 只变更 lifecycle status mirror（proposed -> accepted）、implementation_authority
（none -> contracts，其 permission 仅在本 exact accepted artifact 进入 main 后生效）与 acceptance
provenance；reviewed semantic content（Owner ruling / heading scope / replaced ID trio / SDK
source-runtime SHA / OTHER_UX_CONTRACT_CHANGE / PRODUCT_SEMANTIC_DELTA / Evidence / Acceptance /
Test gate / 正文 whitespace）保持逐字节不变。
