---
spec_id: AGENT_CORE_AGENT_SESSION_MESSAGING_DEPLOYMENT_V2
status: proposed
...
# AGENT_CORE_AGENT_SESSION_MESSAGING_DEPLOYMENT_V2

> **PROPOSED standalone successor / no implementation or production-apply
> authority.** V2 preserves the complete V1 authority except for one declared
> stage-order correction. Until independent exact-head review, Owner
> acceptance, lifecycle-only finalization, merge, and readback, V1 remains the
> current authority and no artifact or production action may consume V2.
...
V2 is a complete standalone successor because the correction changes the
transaction meaning of V1 `CTR-DEP-004/005/006`: it splits file-face apply
(`B1`) from fresh relevant child-generation closure (`B2`) and places B2 in
Stage D, where the accepted ordinary Session/header proof already occurs.
Every other V1 clause is reproduced and remains binding.
...
LATEST_READ_ONLY_MAIN_RECHECK = 379ef227052752a473f6b105d8726e343d2d7f92
AUDITED_RELEASE_SOURCE = 7921f4aecdfe3146210bce7b4a559fe4bc8087cc
AUTH_REPOSITORY_MAIN = ff9e1bec7d364568a92be91f6ffbd49d1d2101de
AUTH_PARENT_AUTHORITY = MINIMAL_AUTH_FOUNDATION_V2
AUTH_SESSION_AUDIENCE_AUTHORITY = AUTH_SERVICE_AGENT_SESSION_MESSAGING_AUDIENCE_CCR_V1
AUTH_TEMP_GRANT_AUTHORITY = AUTH_SERVICE_AGENT_SESSION_MESSAGING_TEMP_GRANT_V1
```
The accepted local product parent owns the complete `agent_session_send` product
contract but explicitly grants no production apply. V2 is a standalone successor
to deployment V1 and does not alter the product parent.

The two exact external Auth authorities named in frontmatter exclusively govern
audience registration/deployment and temporary Grant supply/compensation. This
Spec cannot create, broaden, or replace that cross-repository authority.

## 4. Current State

- `STATE-DEP-001` — Subject: canonical Session Messaging product authority and
  source; revisions: V2 authoring base and latest read-only main recheck
  `379ef227052752a473f6b105d8726e343d2d7f92`; environment: isolated authority
  branch; observed at: `2026-09-04`; basis: `OBS-DEP-001`, `EVD-DEP-001`.
...
- `STATE-DEP-003` — Subject: Auth prerequisites for `agent-session-messaging`;
  revisions: auth main `ff9e1bec7d364568a92be91f6ffbd49d1d2101de`
  registry `1.7.0`, live health registry `1.6.0`, and the two exact external
  authority coordinates in frontmatter; environment: auth repository and
  production health; observed at: `2026-09-04`; basis: `OBS-DEP-003`,
  `EVD-DEP-003`.
- `STATE-DEP-004` — Subject: production deployment authority and release artifact for
  Session Messaging; revision: repository scan at `github/main@379ef227...` and
  exact artifact hashes in `OBS-DEP-004`; environment: Agent Core repository and
  local deployment-artifact root; observed at: `2026-09-04`;
...
- Source revision: accepted implementation lineage from `495b163...`; read-only
  recheck `github/main@379ef227052752a473f6b105d8726e343d2d7f92`
...
### OBS-DEP-003 — Auth audience exists but temporary-Grant production authority does not

- Subject: auth-service audience and temporary Grant authority
- Source revision: auth main `ff9e1bec...`; audience authority `e5a1b8b5...`;
  temporary Grant authority as amended/merged at `ff9e1bec...`
- Environment: auth repository and production health
- Observed at: `2026-09-04`
- Method: inspect exact authority blobs and current registry; read production health
- Result: main registry `1.7.0` contains the exact active machine-only audience and
  scope; production reports registry `1.6.0`, which the accepted temporary-Grant
  authority identifies as the deployed audience face. Fresh Phase-A readback remains
  mandatory. The temporary-Grant Spec freezes the exact tuple and compensation but
  records `production_apply_authority: none`, so it cannot itself authorize C.
- Provenance: auth-service Git objects, exact external authorities, and read-only
  production health; no credential or Grant access.

### OBS-DEP-004 — V1 authority and a reviewed artifact exist, but apply remains blocked

- Subject: Agent Core Specs, reports, and deployment tooling
- Source revision: `github/main@379ef227052752a473f6b105d8726e343d2d7f92`;
  root transaction SHA-256
  `2fe7062809fc450f8d23ab9d260de5975a17e8626db317a3df24ee98887ec120`;
  release manifest `d5056bee50dc280c40c97dcb2c20cd4f95ff1effe3d0d0197a08878b40dad568`;
  rollback manifest `2db59eb53c492486b1cbf2c93b9e9dd23b955775552848e435986b92183797bf`
- Environment: Agent Core repository and local deployment-artifact root
- Observed at: `2026-09-04`
- Method: repository/authority census, artifact hash and simulation checks, independent review
- Result: V1 is accepted with production contracts and the artifact passes all reviewed
  closure checks except the V1 child-generation ordering conflict. V2 remains proposed;
  therefore no production apply is currently authorized from V2.
- Provenance: Git census, exact artifact hashes, simulation transcript, and independent review.

### OBS-DEP-005 — Current main compose includes later Scheduler integration

- Subject: `packages/production-runtime/src/compose.js`
- Source revision: candidate `7921f4a...` blob `cdd3eacf...`; current main
  `379ef227...` blob `c407b064...`
...
### OBS-DEP-006 — V1 fresh-child ordering is not mechanically executable

- Subject: deployment V1 `CTR-DEP-004/005/006` and the exact Stage-B artifact
- Source revision: Agent Core `github/main@379ef227052752a473f6b105d8726e343d2d7f92`;
  deployment V1 blob `ab7a793fef8aa1a8c113282f6b4b54e58d10b008`;
  reviewed root transaction SHA-256
  `2fe7062809fc450f8d23ab9d260de5975a17e8626db317a3df24ee98887ec120`
- Environment: read-only production process/health census plus isolated artifact review
- Observed at: `2026-09-04`
- Method: compare the three contracts, inspect all current parent/child process
  generations and available no-business-write runtime seams, then independently
  review the exact artifact
- Result: V1 requires B to prove a fresh relevant child before C, permits C only
  after all of B passes, and reserves the ordinary source Session/header that can
  start the relevant Router-owned child for D after C. Parent health alone creates
  no such child, and no authorized zero-business-write pre-Grant seam was found.
- Provenance: local read-only process census, health response, source inspection,
  and independent artifact review finding `CTR-DEP-005` as the sole residual blocker.
...
### CLM-DEP-002 — A Grant cannot be applied from tuple authority alone

- Support state: SUPPORTED
- Supported by evidence: `EVD-DEP-003`
- Contradicted by evidence: none known
- Uncertainty: a future exact Auth lifecycle authority may close the production-apply gap.
...
### CLM-DEP-004 — V1 cannot safely authorize the transaction as written

- Support state: SUPPORTED
- Supported by evidence: `EVD-DEP-006`
- Contradicted by evidence: none known
- Uncertainty: Owner acceptance is still required to choose the corrected ordering.
...
- Relation: SUPPORTS
- Bound coordinates: Agent Core implementation lineage `495b163...`, main recheck
  `379ef227...`, observed `2026-09-04`
...
### EVD-DEP-003 — Registry and authority state support the hard Auth prerequisites

- Source observations: `OBS-DEP-003`
- Target: `CLM-DEP-002`
- Relation: SUPPORTS
- Bound coordinates: auth main `ff9e1bec...`, live registry `1.6.0`, exact external
  authority revisions and blobs in frontmatter
- Strength/sufficiency: strong for source registration and the explicit absence of
  temporary-Grant production authority
- Limitations: fresh Phase-A target-bound readback and separate C authority are pending
- Provenance: current registry, external authority documents, and production health.

### EVD-DEP-004 — Repository and artifact census support the current blocked state

- Source observations: `OBS-DEP-004`
- Target: `STATE-DEP-004`
- Relation: SUPPORTS
- Bound coordinates: Agent Core `github/main@379ef227...` and artifact hashes in
  `OBS-DEP-004`
- Strength/sufficiency: strong for current repository authority plus exact local artifact
- Limitations: the artifact is external to Git and must be rehashed immediately before use
- Provenance: repository census, artifact hashes, simulation, and independent review.
...
- Target: `CLM-DEP-003`
- Relation: SUPPORTS
- Bound coordinates: candidate `7921f4a...`; main `379ef227...`; compose blobs in OBS-DEP-005
...
### EVD-DEP-006 — Contract and runtime evidence support the B1/C/D-B2 split

- Source observations: `OBS-DEP-006`
- Target: `CLM-DEP-004`
- Relation: SUPPORTS
- Bound coordinates: Agent Core main `379ef227...`; deployment V1 blob
  `ab7a793f...`; reviewed artifact coordinates recorded in `OBS-DEP-006`
- Strength/sufficiency: strong for the ordering conflict and the absence of an
  authorized pre-Grant child-start seam
- Limitations: does not itself accept V2 or authorize any production action
- Provenance: exact contract comparison, process/health census, source inspection,
  and independent artifact review.
...
### DEC-DEP-005 — Split Stage B into file-face B1 and generation-close B2

- Decision owner: repository Owner
- Decision: B1 applies and verifies the exact file face, parent restart, old-child
  termination, and health; C then supplies the separately authorized temporary
  Grant; D creates the ordinary fresh source/header transaction and proves the
  fresh relevant Router-owned child, thereby closing B2 before any canary.
- Rejected alternative: claim Stage B PASS from parent health, wait for unrelated
  natural traffic, or start a manual CLI child outside the ordinary Session path.
- Reason: the split preserves every identity and fail-closed invariant while making
  the accepted proof mechanically obtainable without business execution before C.
...
feature blobs are present identically in `github/main@379ef227...`. It MUST prove the
...
→ B1 this Agent Core production Authority + immutable artifact + preimage Gate + file-face apply + fresh parent/old-child termination/health
→ C separately authorized minimal Grant supply + positive token/claim proof
→ D fresh runtime/catalog/header proof + fresh relevant Router-owned child generation (closes B2)
→ E one bounded A2A canary
```

A stage MUST NOT borrow authority from later stages. No stage may start before every prior
stage records its expressly required PASS at exact head/runtime coordinates. B1 MAY record
only `STAGE_B_FILE_FACE_APPLIED=PASS` and
`STAGE_B_CHILD_GENERATION=PENDING_STAGE_D`; it MUST NOT report Stage B PASS.
C may start only under `CTR-DEP-006`. D MUST close B2 and `CTR-DEP-005` before E.
Failure MUST stop; apply-time or post-apply failure MUST invoke `CTR-DEP-008`.

### CTR-DEP-005 — Bounded Agent Core apply

Only an authorized actor using an Owner-approved native privileged path MAY atomically replace the 11 present files and create the 6 absent files in §4. Every destination MUST finish as a non-symlink regular file owned `root:wheel` mode `0644`; staged files/directories MUST be fsynced. In B1 the actor MUST restart the Agent Core parent exactly once after complete apply, verify the fresh parent, prove every pre-restart relevant child terminated, and prove health. No unrelated file, config, data, launch definition, or service may change. `CTR-DEP-005` remains PENDING after B1 and reaches PASS only when D records a fresh relevant Router-owned child whose PID, parent PID, UID, trusted Agent identity, process generation, Session/header coordinates, and catalog all bind to the fresh parent and accepted runtime path.

### CTR-DEP-006 — Minimal Grant supply

This clause expressly replaces deployment V1's requirement that all of
`CTR-DEP-005` pass before C. Only after `CTR-DEP-002` PASS and B1 records
`STAGE_B_FILE_FACE_APPLIED=PASS`, the exact accepted external authority
`AUTH_SERVICE_AGENT_SESSION_MESSAGING_TEMP_GRANT_V1`, including its enforcement
amendment, MUST bind the exact tuple and compensation behavior. Because that
external Spec currently records `production_apply_authority: none`, C MUST NOT
start until an exact accepted Auth lifecycle authority expressly permits its
production apply; after that and an Owner gate, the operator MAY grant exact
`(resource=agent-session-messaging, scope=agent.session.send)` to its named
disposable source Agent principal. It MUST NOT grant the target, fleet,
human/service principals, aliases, wildcards, broader scopes, or new credentials.
Phase C, and not Phase A, MUST prove Grant readback and positive token claims.
The same exact external authority governs compensation: on D/E failure or terminal
canary completion, revoke only this temporary Grant with its specified tombstone
semantics and prove enforcement plus absence from effective authorization; retention
requires a later separate accepted activation authority naming principal and purpose.
No compensation may be improvised, and Grant work is outside the 17-file artifact.

### CTR-DEP-007 — Fresh catalog/header and A2A proof

After the verified B1 restart and C PASS, a fresh ordinary source Session/header MUST
start or bind the fresh relevant Router-owned child that closes B2. Evidence MUST record
the new parent PID/generation and child PID, PPID, UID, trusted Agent identity,
`processGeneration`, Session/header coordinates, and catalog, proving the child did not
exist before the restart and belongs to the accepted ordinary runtime path. The fresh
source header and fresh post-restart target header/Run coordinates MUST show
`agent_session_send` exactly once with operation `send`; the messaging aliases MUST be
absent. The target MUST reuse its existing canonical `main` Session, creating it
atomically only if absent; the Gate MUST record which branch occurred and MUST NOT create
a replacement merely to obtain fresh evidence. Across the entire D/E transaction, one
authorized source MUST issue exactly one nonce-marked `agent_session_send` request to a
different target Agent: one Broker request, one parent-RPC/delivery attempt, one receipt,
zero retry, zero replay, and no second receipt even after timeout or `outcome_unknown`.
Evidence MUST bind request/correlation, source, target, canonical `main`, receipt, one
target-owned Run/Turn, terminal outcome, and any bounded reply. The target MUST use
target-owned identity/config/credential; source token, Grant, credential, authorization
header, or authority MUST NOT propagate. Duplicate Run/Turn, automatic reply-send,
external delivery, or hidden second attempt MUST fail.

### CTR-DEP-008 — Failure and rollback

Any failed preimage, integrity, Auth, restart, health, header, child-generation,
identity, delivery, exactly-once, or leakage check MUST fail closed. If Phase C
applied a Grant, D/E failure or terminal completion MUST first invoke only the
separately authorized compensation in `CTR-DEP-006` and prove its tombstone and
effective denial. If files changed, the operator MUST then restore the complete
17-path §4 preimage face and metadata (deleting only the six `ABSENT` preimages),
restart once, and re-prove baseline health/hashes. Evidence/audit rows MAY remain;
secrets MUST NOT be recorded. A Gate, dialog, artifact, restart, child, or receipt
alone MUST NOT be reported as success.

### CTR-GOV-001 — Lifecycle and final-head authority

This proposal authorizes no code, artifact apply, restart, Auth change, Grant, or
canary. After independent exact-head review and explicit Owner authority, one
lifecycle-only docs commit MAY change only this exhaustive allowlist: (1) V2
frontmatter `status: proposed→accepted`, `implementation_authority: none→contracts`,
and `production_apply_authority: none→contracts`; (2) add frontmatter
`accepted_date`, `accepted_by`, `accepted_at`, `accepted_reviewed_head`,
`independent_review_result`, `independent_review_blockers`, `acceptance_verdict`,
`acceptance_semantic_delta`, and `acceptance_authority_basis`; (3) replace only the
Proposed banner with accepted lifecycle/review provenance; (4) V2 footer values
`CURRENT_STATUS`, `IMPLEMENTATION_AUTHORITY`, `PRODUCTION_APPLY_AUTHORITY`, and
...
release paths against current main (16 exact-equal candidate blobs plus separately
reviewed compose delta) and all 17 production preimages/metadata. After the lifecycle
commit, an independent reviewer MUST verify every changed byte is allowlisted; all
non-allowlisted normative bytes are identical; new values match Owner authority and
review identity/outcome; `SEMANTIC_DELTA=DETECTED_AND_INTENDED` binds only
`DEC-DEP-005` and `CTR-DEP-004/005/006/007/008`; and the current-main/17-path
checks remain PASS. `FINAL_HEAD_RECHECK=PASS` binds that exact accepted head; the
head MUST NOT change before merge. Merge supersedes V1 and activates only these
contracts.
...
### ACC-DEP-003 — B1 apply and D/B2 generation closure

- Contracts: `CTR-DEP-004`, `CTR-DEP-005`
- Method: execute audited wrapper after A PASS; verify B1 hashes, parent generation,
  old-child termination, and health; after C PASS, verify the D/B2 child coordinates
- Environment: production Agent Core host under Owner-approved privileged operation
- Required evidence: phase receipts, authorization identity, before/after manifests
  with metadata, restart receipt, fresh parent coordinates, old-child termination,
  B1 pending marker, D/B2 fresh-child coordinates, health output, and unrelated-path
  no-change proof
- Expected result: exactly one complete apply/restart; all 17 targets equal release
  blobs and are regular `root:wheel` `0644`; B1 is not reported as full B PASS;
  D closes B2 with the fresh relevant child
- Failure condition: skipped/out-of-order phase, premature B PASS,
  partial/wrong-metadata face, stale/unbound child, extra restart/change, or failed health.

### ACC-DEP-004 — Grant, header, and canary

- Contracts: `CTR-DEP-006`, `CTR-DEP-007`
- Method: after B1, apply/read back the separately authorized minimal Grant and prove
  positive token claims; run D to close B2 with fresh source/child/header proof; only
  then execute one nonce-bound E request and perform authorized terminal Grant compensation
...
- Required evidence: reviewer identity/verdict, Owner authority, exact allowlist
  diff, non-allowlisted digest, current-main and 17 source/production path proofs,
  external-authority proof, `SEMANTIC_DELTA=DETECTED_AND_INTENDED`,
  `FINAL_HEAD_RECHECK=PASS`
- Expected result: V2 accepted/contracts/contracts, V1 superseded, and only the
  exhaustive README/lifecycle cells changed; exact accepted head remains unchanged
  until merge; no production mutation
...
- `ALT-DEP-002` — Treat the Auth prerequisites as a Grant-only issue: REJECTED;
  audience deployment/readback and separate Grant production authority are both required.
...
- `ALT-DEP-007` — Satisfy fresh-child proof with parent health, unrelated natural
  traffic, or a manually launched CLI: REJECTED; none binds the required ordinary
  Router-owned Session/header path.
- `ALT-DEP-008` — Amend V1 with a partial child document: REJECTED; the semantic
  transaction change requires this complete standalone successor.
...
```text
OPEN_OWNER_DECISIONS = EXACT_HEAD_ACCEPTANCE_AFTER_INDEPENDENT_REVIEW
NORMATIVE_TBD = NONE
UNRESOLVED_AUTHORITY_CONFLICT = RESOLVED_BY_PROPOSAL_PENDING_ACCEPTANCE
PARTIAL_SUPERSESSION = NONE
CURRENT_STATUS = proposed
IMPLEMENTATION_AUTHORITY = none
PRODUCTION_APPLY_AUTHORITY = none
SUPERSEDES = AGENT_CORE_AGENT_SESSION_MESSAGING_DEPLOYMENT_V1_ON_ACCEPTANCE
AUTH_AUDIENCE_AUTHORITY = EXTERNAL_ACCEPTED
AUTH_AUDIENCE_DEPLOYMENT = REQUIRES_FRESH_PHASE_A_PROOF
AUTH_TEMP_GRANT_PRODUCTION_AUTHORITY = MISSING_EXTERNAL_PREREQUISITE
AGENT_CORE_RELEASE_ARTIFACT = BUILT_PENDING_V2_AUTHORITY_AND_FINAL_RECHECK
