...
The builder MUST freeze merged/audited source identities, exact Node distribution and SHA256, Harness version/lock, package-manager version, store identity, app dependencies, native files and runtime executable closure. Build Node, build process and pnpm install process MUST be arm64. Use isolated clean stores/modules or mechanically prove an architecture-safe store; no mixed existing install/rebuild tree. No mutable external symlink or unsealed executable may remain load-bearing. A source stamp MUST report actual source commit and honestly measured dirtyCount; never fabricate zero or copy `.git` to production. Required runtime source deltas MUST be merged; no unmerged patch or wholesale latest-main deployment.
...
| ACC-ARM-003 | CTR-ARM-003 | clean build receipt, source/lock/store/provenance/symlink and executable dependency audit | isolated ARM build | exact v25.6.1; build/install arm64; honest stamp; full sealed dependency closure | dirtyCount fiction, shared mixed install, unmerged delta, mutable load-bearing bytes |
