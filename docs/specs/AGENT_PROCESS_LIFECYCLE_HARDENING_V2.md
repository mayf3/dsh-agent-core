...
# AGENT_PROCESS_LIFECYCLE_HARDENING_V2 — V1 等义 replacement 与 implementation authority

> 状态：**proposed replacement candidate**。
> 本轮：**DOCS-ONLY SPEC AUTHORING**。
> 不 implementation；不修改 production；不重启 Runtime；不修改 Scheduler 或 Scheduler store；不创建 production jobs；不修改 Kernel；不 accepted；不 merge。

## 0. Replacement Authority and Authoring Result

```text
AGENT_PROCESS_LIFECYCLE_HARDENING_V2_AUTHORIZATION_SPEC = PASS
SPEC_STATUS = proposed
DEVELOPMENT_PREFLIGHT_MODE = SUPERSEDE
CHANGE_CLASS = NON_MECHANICAL
IMPLEMENTATION_ALLOWED = NO
READY_FOR_INDEPENDENT_SPEC_REVIEW = YES
```

`PASS` 只表示 replacement Spec authoring 输入和文档闭合；它不表示 V2 已 accepted、implementation 已授权、implementation 已开始、可 merge 或 production 已验证。

### 0.1 Governance preflight

```text
TARGET_REPOSITORY = mayf3/dsh-agent-core
BASE_COMMIT = 0df28d6297a1cb1662eff3face94aaf792a4761d
CURRENT_SPEC = AGENT_PROCESS_LIFECYCLE_HARDENING_V1
CURRENT_SPEC_STATUS = accepted
CURRENT_SPEC_BLOB = fb77790d411cff8135d3326fcfcff5092a2a26c5
CURRENT_IMPLEMENTATION_AUTHORITY = none
DEVELOPMENT_PREFLIGHT_MODE = SUPERSEDE
```

`none -> contracts` 改变 accepted authority 的 authorization semantics。Accepted stable ID 下不得原地改写该语义，所以不得 `AMEND`，不得直接修改 V1 的 `implementation_authority`，必须创建新的完整 replacement Spec ID。

### 0.2 Authorization delta — the only authority-semantic change
...
V1 冻结了产品语义但不授权实现。V2 完整继承同一产品语义。只有 V2 已 `accepted` 且进入 implementation PR 的 base branch 后，V2 Contracts 才可能授权有界实现。该字段不表示代码已经实现，不自动允许 merge；Implementation PR 仍需逐 Contract compliance，production rollout 仍需独立验证。

```text
PRODUCT_SEMANTIC_DELTA = NONE
CONTRACT_SEMANTIC_DELTA = NONE
ACCEPTANCE_SEMANTIC_DELTA = NONE
FAILURE_SEMANTIC_DELTA = NONE
TIMEOUT_SEMANTIC_DELTA = NONE
LIFECYCLE_SEMANTIC_DELTA = NONE
SCHEDULER_SEMANTIC_DELTA = NONE
```

### 0.3 Current authority and future atomic transition

在 V2 accepted 前：

```text
V1_CURRENT_AUTHORITY = YES
V1_STATUS = accepted
V1_IMPLEMENTATION_AUTHORITY = none
V2_STATUS = proposed
V2_IMPLEMENTATION_AUTHORITY_ACTIVE = NO
```

本轮不得修改 V1。未来 V2 acceptance transaction 才允许原子完成：

```text
V2: proposed -> accepted
V1: accepted -> superseded
V2.supersedes = [AGENT_PROCESS_LIFECYCLE_HARDENING_V1]
V1.superseded_by = AGENT_PROCESS_LIFECYCLE_HARDENING_V2
docs/specs/README.md = synchronized
```

### 0.4 V1 review provenance inherited as history, not V2 review

```text
REVIEW_ID = AGENT_PROCESS_LIFECYCLE_HARDENING_V1_ACCEPTANCE_FINAL_HEAD_REVIEW
REVIEWED_SEMANTIC_HEAD = 8475318f12068a28fe937779c91735817b6db9ca
REVIEWED_ACCEPTANCE_HEAD = dcede21ebb4165165ccf9cfdbe291c79623ded99
REVIEWED_FINAL_HEAD = ea21cbfdb3a809be28541be9825a2c4c28a9bc74
SEMANTIC_HEAD_REVIEW = PASS
NINE_REVIEW_FIXES = PASS
FULL_POST_REVIEW_DELTA = MECHANICAL_ONLY
CURRENT_MAIN_COMPATIBILITY = PASS
REQUIRED_FIXES = NONE
```

该 provenance 只证明 V1 的语义与 acceptance 链，并明确 V1 的 `implementation_authority = none`。它不自动构成 V2 review PASS。V2 必须接受新的独立 semantic review，重点检查 whole-Spec semantic equivalence、authorization delta、supersession correctness、Contract / Acceptance crosswalk 与 current-main compatibility。

### 0.5 Current-main compatibility obligation

Required range inspection found no lifecycle-path changes:

```text
SOURCE_RANGE = 34d7c73456f2b177b8ad042e67359bc86fae8861..0df28d6297a1cb1662eff3face94aaf792a4761d
SOURCE_DRIFT = NONE
PRESERVE_PROVIDER_ENV_PROXY_SEAM = YES
```

Any future V2 implementation MUST preserve:

- `RECOGNIZED_PROXY_ENV_KEYS` in `packages/agent-router/src/process.js`;
- proxy stripping in `agentEnv()` before provider injection;
- validated `providerEnv` injection;
- the `AgentProcess` `providerEnv` constructor field;
- `assertTargetProxyRuntime()` startup gate in `packages/production-runtime/src/compose.js`.

These are orthogonal current-main compatibility obligations, not changes to V1 lifecycle product semantics.

### 0.6 Contract ID continuity

```text
CONTRACT_ID_STRATEGY = PRESERVE_C001_TO_C022
```

The full global identity remains `<SPEC_ID>#<CONTRACT_ID>`, so V2 IDs are new global IDs while every textual suffix and semantic meaning remains unchanged.

| V1 Contract | V2 Contract | Semantic disposition |
|---|---|---|
| `V1#C-001` | `V2#C-001` | UNCHANGED |
| `V1#C-002` | `V2#C-002` | UNCHANGED |
| `V1#C-003` | `V2#C-003` | UNCHANGED |
| `V1#C-004` | `V2#C-004` | UNCHANGED |
| `V1#C-005` | `V2#C-005` | UNCHANGED |
| `V1#C-006` | `V2#C-006` | UNCHANGED |
| `V1#C-007` | `V2#C-007` | UNCHANGED |
| `V1#C-008` | `V2#C-008` | UNCHANGED |
| `V1#C-009` | `V2#C-009` | UNCHANGED |
| `V1#C-010` | `V2#C-010` | UNCHANGED |
| `V1#C-011` | `V2#C-011` | UNCHANGED |
| `V1#C-012` | `V2#C-012` | UNCHANGED |
| `V1#C-013` | `V2#C-013` | UNCHANGED |
| `V1#C-014` | `V2#C-014` | UNCHANGED |
| `V1#C-015` | `V2#C-015` | UNCHANGED |
| `V1#C-016` | `V2#C-016` | UNCHANGED |
| `V1#C-017` | `V2#C-017` | UNCHANGED |
| `V1#C-018` | `V2#C-018` | UNCHANGED |
| `V1#C-019` | `V2#C-019` | UNCHANGED |
| `V1#C-020` | `V2#C-020` | UNCHANGED |
| `V1#C-021` | `V2#C-021` | UNCHANGED |
| `V1#C-022` | `V2#C-022` | UNCHANGED |

### 0.7 Acceptance continuity

The complete V1 Future Implementation Acceptance list and §16.1 fault-injection matrix are restated below without semantic change. Numbered Acceptance correspondence is complete:

| V1 Acceptance | V2 Acceptance | Semantic disposition |
|---|---|---|
| `V1 §16 item 1` | `V2 §16 item 1` | UNCHANGED |
| `V1 §16 item 2` | `V2 §16 item 2` | UNCHANGED |
| `V1 §16 item 3` | `V2 §16 item 3` | UNCHANGED |
| `V1 §16 item 4` | `V2 §16 item 4` | UNCHANGED |
| `V1 §16 item 5` | `V2 §16 item 5` | UNCHANGED |
| `V1 §16 item 6` | `V2 §16 item 6` | UNCHANGED |
| `V1 §16 item 7` | `V2 §16 item 7` | UNCHANGED |
| `V1 §16 item 8` | `V2 §16 item 8` | UNCHANGED |
| `V1 §16 item 9` | `V2 §16 item 9` | UNCHANGED |
| `V1 §16 item 10` | `V2 §16 item 10` | UNCHANGED |
| `V1 §16 item 11` | `V2 §16 item 11` | UNCHANGED |
| `V1 §16 item 12` | `V2 §16 item 12` | UNCHANGED |
| `V1 §16 item 13` | `V2 §16 item 13` | UNCHANGED |
| `V1 §16 item 14` | `V2 §16 item 14` | UNCHANGED |
| `V1 §16 item 15` | `V2 §16 item 15` | UNCHANGED |
| `V1 §16 item 16` | `V2 §16 item 16` | UNCHANGED |
| `V1 §16 item 17` | `V2 §16 item 17` | UNCHANGED |
| `V1 §16 item 18` | `V2 §16 item 18` | UNCHANGED |
| `V1 §16 item 19` | `V2 §16 item 19` | UNCHANGED |
| `V1 §16 item 20` | `V2 §16 item 20` | UNCHANGED |
| `V1 §16 item 21` | `V2 §16 item 21` | UNCHANGED |
| `V1 §16 item 22` | `V2 §16 item 22` | UNCHANGED |
| `V1 §16 item 23` | `V2 §16 item 23` | UNCHANGED |
| `V1 §16 item 24` | `V2 §16 item 24` | UNCHANGED |
| `V1 §16 item 25` | `V2 §16 item 25` | UNCHANGED |
| `V1 §16 item 26` | `V2 §16 item 26` | UNCHANGED |
| `V1 §16 item 27` | `V2 §16 item 27` | UNCHANGED |
| `V1 §16 item 28` | `V2 §16 item 28` | UNCHANGED |
| `V1 §16 item 29` | `V2 §16 item 29` | UNCHANGED |
| `V1 §16 item 30` | `V2 §16 item 30` | UNCHANGED |
| `V1 §16 item 31` | `V2 §16 item 31` | UNCHANGED |
| `V1 §16 item 32` | `V2 §16 item 32` | UNCHANGED |
| `V1 §16 item 33` | `V2 §16 item 33` | UNCHANGED |
| `V1 §16 item 34` | `V2 §16 item 34` | UNCHANGED |
| `V1 §16 item 35` | `V2 §16 item 35` | UNCHANGED |
| `V1 §16 item 36` | `V2 §16 item 36` | UNCHANGED |
| `V1 §16 item 37` | `V2 §16 item 37` | UNCHANGED |
| `V1 §16 item 38` | `V2 §16 item 38` | UNCHANGED |
| `V1 §16 item 39` | `V2 §16 item 39` | UNCHANGED |

All §16.1 fault-case IDs, fixtures, injections, unique oracles, exact counters, snapshots, and final reconciliation expectations are copied unchanged. No previously passing V1 implementation may pass or fail differently because of V2 Acceptance text; the only authorization delta is `none -> contracts`.
...
V2 继续保证：不永久挂起、不错误复用、不自动重放、不把“停止等待”冒充“执行已终止”。
...
Replacement child Spec candidate = AGENT_PROCESS_LIFECYCLE_HARDENING_V2 (proposed)
Implementation authority = CONTRACTS_IF_AND_ONLY_IF_ACCEPTED_AND_PRESENT_IN_IMPLEMENTATION_BASE
```
...
## 3. Current State, Observations, Claims, and Evidence

### STATE-001 — Accepted V1 remains current while V2 is proposed

- Subject: AgentProcess lifecycle governing authority in `mayf3/dsh-agent-core`.
- As of commit: `0df28d6297a1cb1662eff3face94aaf792a4761d`.
- Environment: `origin/main`, observed during docs-only authoring.
- Projection: V1 is `accepted`, has `implementation_authority: none`, and remains current; V2 is a proposed replacement candidate and has no active implementation authority.
- Basis: `OBS-001`, `OBS-002`, `CLM-001`, `EVD-001`.

### STATE-002 — Current main preserves the provider proxy seam

- Subject: existing AgentProcess and production-runtime compatibility behavior.
- As of commit: `0df28d6297a1cb1662eff3face94aaf792a4761d`.
- Environment: `origin/main`, source inspection.
- Projection: proxy keys are stripped before validated per-target `providerEnv` injection, `AgentProcess` retains its `providerEnv` constructor field, and production compose retains the `assertTargetProxyRuntime()` startup gate.
- Basis: `OBS-003`, `OBS-004`, `CLM-002`, `EVD-002`.

### OBS-001 — Accepted V1 identity and bytes

- Subject: `docs/specs/AGENT_PROCESS_LIFECYCLE_HARDENING_V1.md`.
- Source revision: `0df28d6297a1cb1662eff3face94aaf792a4761d`.
- Environment: `origin/main`.
- Method: `git rev-parse HEAD:docs/specs/AGENT_PROCESS_LIFECYCLE_HARDENING_V1.md` and full-file read.
- Result: blob `fb77790d411cff8135d3326fcfcff5092a2a26c5`; status `accepted`; implementation authority `none`.
- Provenance: Git object database and accepted V1 file.

### OBS-002 — Governance requires supersession for the authority change

- Subject: repository governance and Spec format.
- Source revision: `0df28d6297a1cb1662eff3face94aaf792a4761d`.
- Environment: linked authoring worktree.
- Method: full read of the governance preflight, authoring, protocol, and format authorities named in this task.
- Result: changing accepted authorization semantics from `none` to `contracts` cannot be an in-place accepted-ID rewrite; a new complete replacement Spec is required.
- Provenance: `.agents/README.md`, `.agents/protocol/SPEC_GOVERNANCE_V0.md`, `.agents/protocol/SPEC_FORMAT_V0.md`, and `.agents/skills/spec-governance/modes/{PREFLIGHT,AUTHOR}.md`.

### OBS-003 — No material lifecycle source drift in the required range

- Subject: AgentProcess lifecycle-related source.
- Source range: `34d7c73456f2b177b8ad042e67359bc86fae8861..0df28d6297a1cb1662eff3face94aaf792a4761d`.
- Environment: `origin/main`.
- Method: path-scoped Git log and diff for `packages/agent-router/src/process.js`, `packages/agent-router/src/index.js`, and `packages/production-runtime/src/compose.js`.
- Result: no commits and no diff in the scoped paths; `SOURCE_DRIFT = NONE`.
- Provenance: Git range inspection in the fresh linked worktree.

### OBS-004 — Provider environment compatibility seam is present

- Subject: provider proxy isolation and startup validation.
- Source revision: `0df28d6297a1cb1662eff3face94aaf792a4761d`.
- Environment: `origin/main`.
- Method: source inspection.
- Result: `process.js` retains `RECOGNIZED_PROXY_ENV_KEYS`, `agentEnv()` proxy stripping, validated `providerEnv` injection, and the `AgentProcess.providerEnv` constructor field; `compose.js` retains `assertTargetProxyRuntime()` and invokes it during startup.
- Provenance: `packages/agent-router/src/process.js` and `packages/production-runtime/src/compose.js`.

### CLM-001 — SUPERSEDE is the only valid authority disposition

- Support state: SUPPORTED.
- Supported by evidence: `EVD-001`.
- Uncertainty: none for this authoring classification.

### CLM-002 — V2 can preserve V1 product semantics on current main

- Support state: SUPPORTED.
- Supported by evidence: `EVD-002`.
- Uncertainty: V2 still requires a new independent whole-Spec semantic review before acceptance.

### EVD-001 — Accepted V1 plus governance supports supersession

- Source observations: `OBS-001`, `OBS-002`.
- Target: `CLM-001`, `STATE-001`.
- Relation: SUPPORTS.
- Bound coordinates: repository `mayf3/dsh-agent-core`, main `0df28d6297a1cb1662eff3face94aaf792a4761d`, V1 blob `fb77790d411cff8135d3326fcfcff5092a2a26c5`.
- Strength/sufficiency: sufficient for authoring-mode and current-authority classification.
- Limitations: does not accept V2 and does not authorize implementation.
- Provenance: accepted V1 and repository governance authorities.

### EVD-002 — Source inspection supports semantic compatibility

- Source observations: `OBS-003`, `OBS-004`.
- Target: `CLM-002`, `STATE-002`.
- Relation: SUPPORTS.
- Bound coordinates: source range `34d7c73456f2b177b8ad042e67359bc86fae8861..0df28d6297a1cb1662eff3face94aaf792a4761d`.
- Strength/sufficiency: sufficient to classify required lifecycle source drift as `NONE` and the existing provider proxy seam as present.
- Limitations: does not replace independent V2 semantic review or future implementation conformance evidence.
- Provenance: Git range inspection and source reads.

### Existing current-evidence details preserved from V1
...
### DEC-001 — Preserve the complete V1 product contract

- Decision owner: repository maintainers.
- Decision: V2 restates the complete V1 Current Truth, C-001 through C-022, Future Implementation Acceptance items 1 through 39, the fault-injection matrix, failure semantics, timeout semantics, lifecycle semantics, and Scheduler boundary without product-semantic change.
- Rejected alternative: use the authority change to revise, simplify, renumber, narrow, or expand AgentProcess behavior.
- Reason: this replacement has exactly one authorization delta.
- Owner input remaining: NONE.

### DEC-002 — Change only conditional implementation authority

- Decision owner: repository maintainers.
- Decision: V2 declares `implementation_authority: contracts`, which can become effective only after V2 is accepted and is present in an implementation PR base branch.
- Rejected alternative: treat proposed status, a Draft PR, or this authoring commit as implementation authorization.
- Reason: Spec lifecycle and implementation authority are separate gates.
- Owner input remaining: NONE.

### DEC-003 — Preserve atomic whole-authority supersession

- Decision owner: repository maintainers.
- Decision: while V2 is proposed, V1 remains accepted/current and unchanged. A future acceptance transaction must atomically accept V2, mark V1 superseded, add the V1 backlink, and synchronize the index.
- Rejected alternative: prematurely mark V1 superseded in this authoring PR or simulate partial supersession.
- Reason: proposed authority cannot displace accepted authority.
- Owner input remaining: NONE.

### DEC-004 — Preserve orthogonal provider proxy behavior

- Decision owner: repository maintainers.
- Decision: any future lifecycle implementation must preserve the current provider environment proxy seam and production startup gate.
- Rejected alternative: remove or regress these orthogonal behaviors during lifecycle refactoring.
- Reason: current main contains valid independent work outside the lifecycle semantic delta.
- Owner input remaining: NONE.
...
### 16.2 Inherited V1 amendment closure crosswalk
...
BASE_MAIN = 0df28d6297a1cb1662eff3face94aaf792a4761d
CURRENT_V1_SPEC_BLOB = fb77790d411cff8135d3326fcfcff5092a2a26c5
DEVELOPMENT_PREFLIGHT_MODE = SUPERSEDE
AUTHORITY_DISPOSITION = SUPERSEDE
...
V1_CURRENT_AUTHORITY = YES
V1_IMPLEMENTATION_AUTHORITY = none
V2_SPEC_ID = AGENT_PROCESS_LIFECYCLE_HARDENING_V2
V2_STATUS = proposed
V2_IMPLEMENTATION_AUTHORITY = contracts
V2_IMPLEMENTATION_AUTHORITY_ACTIVE = NO
...
CONTRACT_SEMANTIC_DELTA = NONE
ACCEPTANCE_SEMANTIC_DELTA = NONE
AUTHORIZATION_DELTA = none -> contracts
SOURCE_DRIFT = NONE
PRESERVE_PROVIDER_ENV_PROXY_SEAM = YES
...
SCHEDULER_STORE_CHANGE = NONE
PRODUCTION_JOBS_CHANGE = NONE
KERNEL_CHANGE = NONE
MERGE = NO
READY_FOR_INDEPENDENT_SPEC_REVIEW = YES
```
