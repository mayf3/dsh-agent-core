...
- frozen candidate SHA-256: recorded in §7; the original review request targets
  the historical r1 SHA and does not review this amendment
...
- prospective root-controlled cutover + whole-host quiescence proof bundle（受控停机回执[如涉及]、
  完整 root 进程普查零 runtime 树成员、workspace/session holder 零占用、
  epoch retirement、exact record preimage、exclusive window and one startup
  nonce 绑定，全部 fail-closed 校验）；pre-floor historical restarts may be
  eligible only after the current causal cut; no historical floor proof is
  manufactured；
...
1. `ROUTER_RESTART_SAFETY = PROVEN`：部署二进制含 durable generation floor
   ...
   该门只约束 prospective recovery stop/restart；不能倒推 s256 过去
   pre-floor restart 合规。Spec RQ-001/003/005 另要求完整的当前 quiescence
   cutover + 不可恢复旧执行的独占窗口证明。
...
spec §6（ACC-RQ-001..010，含 pre-floor s256 prospective cutover、跨 epoch
bare generation 复用、三记录独立结算、端到端 scheduler 收敛、preimage
回滚、validator 顺序）与 §7（NEG-RQ-001..019：bundle 缺失/篡改/custody 无效、
...
Only after independent B4 review returns PASS with BLOCKERS=NONE on this
exact candidate SHA may the Owner answer Q1/Q2. Any NO leaves the candidate
draft and no implementation proceeds:
...
| M2 trusted evidence or exact live ownership, never guesswork | RQ-001–004 propose one trusted prospective cutover evidence class under Owner acceptance; §3.3 keeps PID/time/new-generation shortcuts rejected. Pre-floor history is not backfilled. |
| M3 UNKNOWN fails closed | RQ-003 V1–V10 treats UNKNOWN as invalid; zero-write and structured failure. |
| M4 single-writer exclusivity | RQ-003/005 require a root-controlled exclusive stop–census–single-startup window through consumption; the startup barrier remains closed, with per-record preimage, restore-on-failure and existing crash cleanup. This substitutes for the packet's external runner. |
...
NEXT_SINGLE_ACTION = independent B4-only delta review of this amended frozen SHA;
