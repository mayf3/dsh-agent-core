...
WORKFLOW_LIVE_CHAIN            = launchd com.openclaw.workflow-dispatcher (30min)
                                  → restored unified-dispatcher.py / check-dispatch-health.py
                                  → openclaw cron add → ~/.openclaw/cron/jobs.json
                                  → OpenClaw gateway (com.openclaw.gateway PID 31652)
                                  → agentTurn → runs/ records
FORUM_LIVE_CHAIN               = launchd com.openclaw.forum-scheduler (60min)
...
| unified-dispatcher.py | `~/.openclaw/groups/workspace-oc_648db8f3df0ef0249b761ebb0b7a56ab/skills/cron-domain-scheduler/scripts/unified-dispatcher.py` | `unified-dispatcher.py.bak-caller-migration-v1-20260815-121248` | **migrated**（agentcore-cron add；dedup 读 agentcore store） | **pre-migration**（openclaw cron add --at） |
...
```
37723  0  com.openclaw.host-exec-runner
31652  0  com.openclaw.gateway            ← OpenClaw 网关（执行 cron job 的进程），RUNNING
   0   0  com.openclaw.forum-scheduler    ← 已加载，周期触发（日志显示 ~每小时：10:41/11:42/12:43），上次 exit=0
   0   0  com.openclaw.workflow-dispatcher ← 已加载，周期触发（日志显示 ~每 30 分钟：…/11:43/12:13/12:43），上次 exit=0
```

三个 daemon 均为 ACTIVE（加载于 system 域；PID=0 表示周期型任务当前不在执行瞬间，
非未加载）。另外 `com.openclaw.agent-node-502`(5335)、`com.openclaw.control-api`(64953)、
`com.openclaw.auto-repair`(64950) 在线。

### 1.3 是否已向 `~/.agent-core/scheduler/` 写入新 job —— 是（且这正是风险实证）

- `~/.agent-core/scheduler/jobs.json`（8508 B，authsvc:staff，12:43 写入）含 **5 个
...
- 来源：`com.openclaw.workflow-dispatcher.log` 12:43:52 运行（迁移版）Step 2
...
# 3) 恢复 forum-scheduler.sh 的 ACL（launchd 以 UID505 authsvc 执行；bak 无 ACL）：
...
- 12:52:55 手动执行 `workflow-dispatcher-launchd.sh`（恢复版）：
  - Step 1 unified-dispatcher.py（旧版）正常执行（token 读取受限属 yanfenma 身份
    限制，daemon 以 authsvc 运行时有凭据；见 12:13 日志对照）；
...
**workflow-dispatcher daemon 13:13:55 运行（恢复版，authsvc，`com.openclaw.workflow-dispatcher.log`）：**
...
**forum-scheduler daemon 13:43:22 运行（恢复版 v5，authsvc，`forum-scheduler.log`）：**
...
- `com.openclaw.gateway` RUNNING（PID 31652）；`com.openclaw.agent-node-502`（5335）。
...
- 这 5 个 job 于 12:58:28 全部 **finished status=ok**（`~/.openclaw/cron/runs/*.jsonl`，
  authsvc 写入），其中包含真实 workflow 业务推进：
