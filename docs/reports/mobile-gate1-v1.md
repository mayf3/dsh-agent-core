...
| `POST /v1/switch-agent` `{surfaceId, targetAgentId, sessionId?}` | 更新 Binding；session 决策全在 Router | `agentRouter.switchAgent` |
...
- `GATE1_EXPLICIT_SESSION_SWITCH`：显式 sessionId → A/work，ALPHA-2；
