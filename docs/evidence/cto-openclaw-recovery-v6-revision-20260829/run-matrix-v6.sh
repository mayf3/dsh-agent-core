#!/bin/bash
# REVISION-ONLY driver: re-runs the v6 runner's synthetic sandbox matrix (same 15
# scenarios and expectations as the independent v5 audit's run-matrix.sh).
# Runs as uid 502 (no sudo anywhere); each case is `bash <v6 runner> --synthetic-case <name>`.
set -u
RUNNER=/tmp/run-agent-core-cto-openclaw-recovery-v6.sh
EV=/tmp/cto-v6-revision-evidence/matrix
PASS=0; FAIL=0
mkdir -p "$EV"
...
run_case() {
  ...
  if [ "$name" = "fault-env-pollution-ignored" ]; then
    env_prefix="RECOVERY_ATOMIC_TEST_FAULT=rename RECOVERY_ATOMIC_TEST_CANDIDATE=/tmp/cto-v6-revision-evidence/should-never-be-used"
  fi
