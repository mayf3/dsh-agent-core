...
function makeAudit() {
  ...
  return {
    ...
    // AMENDMENT_1 §5.3 (implementation era): the read-only reconcile lookup
    // surface is part of the provider contract; stubbed here — the lookup
    // path itself is exercised end-to-end by the broker/agent-router suites.
    findInvocation: () => ({ intentFound: false, outcome: null, oldestRetainedIntentTs: null, retentionIntegrity: 'clean' }),
  }
}
...
// Implementation era (AMENDMENT_1 §5.3): the relay issues EXACTLY ONE read-only
// reconcile lookup; the parent's retained L1 truth (intent + outcome accepted)
// reconciles the lost response to DELIVERED — zero redelivery.
results.push(await runCase('A: receipt committed + parent RPC response lost -> reconcile DELIVERED', async () => {
  const legs = []
  const handlers = createRelayHandlers(agentSessionMessagingManifest, async (call) => {
    ...
    if (call.capabilityId === 'agent_session_send_reconcile') {
      // Parent-side L1 truth retained: intent + outcome(accepted).
      return { ok: true, result: { ok: true, result: { invocationCorrelationFound: true, outcome: { result: 'accepted' }, oldestRetainedIntentTs: 1, retentionIntegrity: 'clean' } } }
    }
  ...
  assert.equal(sendLegs.length, 1, 'the send executed exactly once — zero redelivery')
  ...
  return {
    ...
    dims: { delivery: 'DELIVERED (reconciled)', reply: 'NOT_WAITED' },
    extra: 'EXACTLY_ONCE: one inbox receipt, one target Run; NO_AUTOMATIC_RETRY holds (lookup is read-only, the send is never replayed)',
  }
}))

// CASE G2 — parent answered, but the envelope is unusable AND the lookup is
// dark: the honest ambiguous envelope is terminal (UNKNOWN + UNKNOWN), never
// a fabricated dimension, never a replay.
results.push(await runCase('G2: unusable parent envelope + dark lookup -> ambiguous terminal', async () => {
  const legs = []
  const handlers = createRelayHandlers(agentSessionMessagingManifest, async (call) => {
    legs.push(call)
    throw new Error('rpc channel dark')
  })
  const relayed = await handlers.send('send', ARGS)
  const sendLegs = legs.filter((c) => c.capabilityId === 'agent_session_send')
  const lookupLegs = legs.filter((c) => c.capabilityId === 'agent_session_send_reconcile')
  assert.equal(sendLegs.length, 1)
  assert.equal(lookupLegs.length, 1)
  return {
    caller: JSON.stringify(relayed),
    dims: { delivery: 'UNKNOWN (explicit)', reply: 'UNKNOWN' },
    extra: 'STILL_UNKNOWN is explicit and final — NO blind replay; the caller may only issue a NEW intentional send',
  }
}))
...
console.log('  DELIVERY_CAN_BE_RECONCILED=YES — delivery is already PROVEN.')
console.log('FROZEN CANDIDATE (impl @ 1d70571): the expression gap is closed (§5.2 render')
console.log('  detail + failureCode/reason persistence) and the outcome_unknown seam is')
console.log('  reconciled read-only (§5.3 anchor + agent_session_send_reconcile — CASE A')
console.log('  above now settles DELIVERED with zero redelivery).')
