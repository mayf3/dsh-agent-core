...
run_case() {
  ...
  if [ "$name" = "fault-env-pollution-ignored" ]; then
    env_prefix="RECOVERY_ATOMIC_TEST_FAULT=rename RECOVERY_ATOMIC_TEST_CANDIDATE=/tmp/cto-v5-audit-evidence/should-never-be-used"
