...
SERVICE_DIR='/Users/yanfenma/.local/services/svc-workflow'
...
service_pid() { /bin/launchctl print "gui/$(id -u)/$LABEL" 2>/dev/null | awk '/^[[:space:]]*pid =/{print $3; exit}'; }
restart_service() { /bin/launchctl kickstart -k "gui/$(id -u)/$LABEL"; }
...
if bash "$RELEASE_SH" deploy "$MAIN_COMMIT" 2>&1 | tee -a "$LOG"; then
