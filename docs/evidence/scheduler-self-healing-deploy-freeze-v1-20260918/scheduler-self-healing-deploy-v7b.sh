#!/bin/bash
# =============================================================================
# scheduler-self-healing-deploy-v7b.sh — SCHEDULER_SELF_HEALING_FROM_FEISHU_V1
# Phase C APP-ONLY SEALED GENERATION **FOLLOW-UP** (target 41f354d, ONE file).
#
# WHY V7B EXISTS: the V7 sealed-generation swap (DEPLOYMENT=PASS 11:35:07Z
# 2026-09-18) deployed packages/scheduler only — exactly its frozen scope.
# Postdeploy acceptance then found the app closure still carried a STALE
# broker manifest: packages/broker/src/capabilities/self-ops.js (62905c1-era,
# status+reconcile_turn only) while the accepted spec §3 (C-SH-003) requires
# the job_disposition operation (41f354d bytes, commit 7e66c79, sha b302810c…).
# V7B ships exactly that ONE file as a sealed app generation swap. NOTHING
# else changes: scheduler bytes, root package.json, harness, node-runtime,
# home, config, credentials, store — all pinned unchanged by gates below.
#
# V7 principles carried over (Owner ruling 2026-09-18): NO trusted-root
# reinstall, NO package manager, NO network fetch, NO corepack. Atomic
# renames in ONE parent dir. Any post-swap failure => automatic rollback.
# A PATH shim makes pnpm/corepack/npm/npx/yarn exit 99 if ever invoked.
#
# Modes:
...
FROZEN_BROKER_SELFOPS_SHA="b302810c29c92404fcf538819af5bb997410c395fd42d10e93377729d221907a"
BROKER_REL="packages/broker/src/capabilities/self-ops.js"
SOURCE_FILE="${SOURCE_FILE:-/Users/yanfenma/workspace/project/dsh-agent-core-selfheal-impl/packages/broker/src/capabilities/self-ops.js}"
FROZEN_PREIMAGE_APP_MANIFEST_SHA="c41a90453d0170085b3959943af70b1111c9403b5191e097649e28c01c4e15b7"
FROZEN_TARGET_APP_MANIFEST_SHA="f261066d77dedb2240c2e328eebbccd26ce7e9a97055212874c0616d248a1a5e"
...
STATE_ROOT="/usr/local/var/agent-core/scheduler-self-healing-v7b-apponly"
RB_TMP="/private/tmp/scheduler-self-healing-deploy-v7b-receipt"
...
PM_TRIPFILE=""
setup_pm_guard() {
  PM_GUARD_DIR="$(mktemp -d /tmp/scheduler-v7b-pm-shim.XXXXXX)"
...
fixture_selftest() {
  local S; S="$(mktemp -d /tmp/scheduler-v7b-fixture.XXXXXX)"
  setup_pm_guard
  say "fixture sandbox: $S"
  # live-like app: OLD broker manifest (no job_disposition marks the old
  # generation), NEW scheduler bytes (v7 already shipped them), untouched
  # sibling capability file, root package.json.
  mkdir -p "$S/app/packages/broker/src/capabilities" "$S/app/packages/scheduler/src"
  echo old-manifest-no-jobdisposition > "$S/app/packages/broker/src/capabilities/self-ops.js"
  echo new-scheduler-bytes > "$S/app/packages/scheduler/src/scheduler.js"
  echo pkg-v7 > "$S/app/package.json"
  echo untouched-sibling > "$S/app/packages/broker/src/capabilities/workflow.js"
  printf 'new-manifest-with-job_disposition\n' > "$S/source-self-ops.js"
  ...
  # than build_app_next_v7b) — live tree with exactly ONE file replaced — then
  # aggregated with the same manifest pipeline. A build bug cannot self-confirm
  # through this path.
  local etree="$S/expected-tree"
  mkdir -p "$etree"
  cp -a "$S/app/." "$etree/"
  cp "$S/source-self-ops.js" "$etree/packages/broker/src/capabilities/self-ops.js"
  expected="$(manifest_of "$etree")"

  build_app_next_v7b "$S/app" "$S/source-self-ops.js" "$S/app.next" || { echo "FIXTURE=FAIL build"; exit 1; }
  ...
  grep -q 'job_disposition' "$S/app.next/$BROKER_REL" || { echo "FIXTURE=FAIL new bytes"; exit 1; }
  ...
  [ "$(cat "$S/app.next/package.json")" = "pkg-v7" ] || { echo "FIXTURE=FAIL root pkg must be untouched"; exit 1; }
  ...
  rb="$(swap_generations "$S/app" "$S/app.next" "$S/app.rollback-v7b-test")" || { echo "FIXTURE=FAIL swap"; exit 1; }
  [ -d "$S/app" ] && [ -d "$S/app.rollback-v7b-test" ] || { echo "FIXTURE=FAIL swap layout"; exit 1; }
  [ "$(manifest_of "$S/app")" = "$built" ] || { echo "FIXTURE=FAIL post-swap identity"; exit 1; }
  # rollback restore (Owner §6)
  restore_rollback "$S/app" "$S/app.rollback-v7b-test" "$S/app.failed-v7b-test" || { echo "FIXTURE=FAIL restore"; exit 1; }
  ...
  echo "FIXTURE=PASS (pm-guard clean, manifest-exact single-file build, scheduler/pkg untouched, swap+restore round-trip OK)"
  rm -rf "$S"
}
...
production_main() {
  ...
  local AUTH_MARKER="$STATE_ROOT/receipts/deploy-v7b.auth"
  ...
  if ! ( set -o noclobber; printf 'packet=FREEZE_V7B\ntarget=%s\nauthorized_by=mayf3\ncreated=%s\n' \
  ...
  post_swap_fail() { # $1 = reason ; automatic rollback per Owner §6
    ...
    if [ "$rollback_done" = "0" ] && [ -n "$app_rb" ] && [ -d "$app_rb" ]; then
      restore_rollback "$APP" "$app_rb" "$APP.failed-v7b-$TS" \
    ...
    echo "DEPLOY=NO STOP=YES (rolled back to the pre-V7B generation)" >&2
    exit 1
  }

  # G1 source file pin (the single file v7b ships)
  [ -f "$SOURCE_FILE" ] || fail "G1 source file missing"
  local src_sha="$(file_sha "$SOURCE_FILE")"
  [ "$src_sha" = "$FROZEN_BROKER_SELFOPS_SHA" ] || fail "G1 source file pin mismatch ($src_sha)"
  grep -q 'job_disposition' "$SOURCE_FILE" || fail "G1 source file missing job_disposition"
  say "G1 source file bound (41f354d broker self-ops manifest, ONE-file scope)"

  # G2 live app = the V7 generation (no drift since 11:35:07Z)
  local live_man="$(manifest_of "$APP")"
  [ "$live_man" = "$FROZEN_PREIMAGE_APP_MANIFEST_SHA" ] || fail "G2 live app manifest drifted ($live_man)"
  say "G2 live app manifest matches the post-V7 generation"
  ...
  # sudo/bash/tee command lines all contain "deploy-v7b") is excluded; a
  # concurrent v7b double-run is already impossible past the G0 noclobber
  # marker, so G6b only guards OTHER families — including a stale V7 rerun
  # and any installer-lineage process (Owner rule: fail-closed, never race).
  local g6b_hits
  g6b_hits="$(pgrep -fl 'trusted-cp-deploy-install|run-authorized-transaction|run-routing-install|scheduler-self-healing-deploy' 2>/dev/null | grep -v 'deploy-v7b' || true)"
  if [ -n "$g6b_hits" ]; then
    printf '%s\n' "$g6b_hits"
  ...
  setup_pm_guard
  local next="$TRUSTED_ROOT/app.next-v7b-$TS"
  build_app_next_v7b "$APP" "$SOURCE_FILE" "$next" || fail "G9 app.next build failed"
  local built_man="$(manifest_of "$next")"
  [ "$built_man" = "$FROZEN_TARGET_APP_MANIFEST_SHA" ] || fail "G9 built app.next manifest != frozen v7b target ($built_man)"
  local broker_sha="$(file_sha "$next/$BROKER_REL")"
  [ "$broker_sha" = "$FROZEN_BROKER_SELFOPS_SHA" ] || fail "G9 broker self-ops != frozen target"
  grep -q 'job_disposition' "$next/$BROKER_REL" || fail "G9 broker self-ops missing job_disposition"
  local sched_sha="$(file_sha "$next/packages/scheduler/src/scheduler.js")"
  [ "$sched_sha" = "$FROZEN_SCHEDULER_JS_SHA" ] || fail "G9 scheduler.js drifted (v7b must not touch scheduler)"
  local pkg_sha="$(file_sha "$next/package.json")"
  [ "$pkg_sha" = "$FROZEN_TARGET_ROOT_PKG_SHA" ] || fail "G9 root package.json drifted"
  ...
  if ! swap_out="$(swap_generations "$APP" "$next" "$app_rb")"; then
    ...
    [ -d "$APP" ] || { echo "FATAL: swap failed and restore failed — pre-V7B generation preserved at $app_rb; manual recovery required" >&2; exit 1; }
    fail "APPLY swap failed (restored pre-V7B generation)"
  fi
  say "APPLY swapped: V7 generation preserved at $app_rb"
  ...
  [ "$dep_sched" = "$FROZEN_SCHEDULER_JS_SHA" ] || post_swap_fail "deployed scheduler.js changed (v7b must not touch scheduler)"
  local dep_broker="$(file_sha "$APP/$BROKER_REL")"
  [ "$dep_broker" = "$FROZEN_BROKER_SELFOPS_SHA" ] || post_swap_fail "deployed broker self-ops != frozen v7b target"
  grep -q 'job_disposition' "$APP/$BROKER_REL" || post_swap_fail "deployed broker self-ops missing job_disposition"
  ...
  pm_guard_clean || post_swap_fail "package manager invoked during deployment"
  {
    ...
    echo "V7B_SCOPE=$BROKER_REL ONLY"
    echo "PRIOR_GENERATION_MANIFEST=$FROZEN_PREIMAGE_APP_MANIFEST_SHA (V7, DEPLOYMENT=PASS 2026-09-18T11:35:07Z)"
    echo "DEPLOYED_APP_MANIFEST_SHA=$(manifest_of "$APP")"
    echo "DEPLOYED_BROKER_SELFOPS_SHA=$dep_broker"
    ...
    echo "ROLLBACK=mv $APP $APP.failed-v7b-\$TS && mv $app_rb $APP && launchctl kickstart -k system/ai.agent-core.runtime"
  ...
  cp "$RB_TMP/deploy-receipt.txt" "$STATE_ROOT/receipts/deploy-v7b.receipt" \
