#!/bin/bash
# =============================================================================
# scheduler-self-healing-deploy-v8.sh — SCHEDULER_SELF_HEALING_FROM_FEISHU_V1
...
#
# WHY V8 EXISTS: AGENT_CORE_SELF_SERVICE_SCHEDULER_TOOLS_V4 (accepted a946be2)
# superseded the infrastructure=true flag on self_ops (Owner ruling
# SELF_OPS_MODEL_VISIBLE=YES) — the broker registry's generic filter
# (infrastructure !== true) kept self_ops out of the model-visible tool list
# in every lineage. V8 ships exactly ONE file as a sealed app generation swap:
...
STATE_ROOT="/usr/local/var/agent-core/scheduler-self-healing-v8-apponly"
RB_TMP="/private/tmp/scheduler-self-healing-deploy-v8-receipt"
...
PM_TRIPFILE=""
setup_pm_guard() {
  PM_GUARD_DIR="$(mktemp -d /tmp/scheduler-v8-pm-shim.XXXXXX)"
...
build_app_next_v8() { # $1=live_app  $2=source_root  $3=out_dir
...
fixture_selftest() {
  local S; S="$(mktemp -d /tmp/scheduler-v8-fixture.XXXXXX)"
  setup_pm_guard
  ...
  # than build_app_next_v8) — live tree with exactly ONE file replaced —
  ...
  build_app_next_v8 "$S/app" "$S/srcroot" "$S/app.next" || { echo "FIXTURE=FAIL build"; exit 1; }
  ...
  rb="$(swap_generations "$S/app" "$S/app.next" "$S/app.rollback-v8-test")" || { echo "FIXTURE=FAIL swap"; exit 1; }
  [ -d "$S/app" ] && [ -d "$S/app.rollback-v8-test" ] || { echo "FIXTURE=FAIL swap layout"; exit 1; }
  [ "$(manifest_of "$S/app")" = "$built" ] || { echo "FIXTURE=FAIL post-swap identity"; exit 1; }
  # rollback restore (Owner §6)
  restore_rollback "$S/app" "$S/app.rollback-v8-test" "$S/app.failed-v8-test" || { echo "FIXTURE=FAIL restore"; exit 1; }
...
production_main() {
  ...
  local AUTH_MARKER="$STATE_ROOT/receipts/deploy-v8.auth"
  ...
  if ! ( set -o noclobber; printf 'packet=FREEZE_V8\ntarget=%s\nauthorized_by=mayf3\ncreated=%s\n' \
  ...
  post_swap_fail() { # $1 = reason ; automatic rollback per Owner §6
    ...
    if [ "$rollback_done" = "0" ] && [ -n "$app_rb" ] && [ -d "$app_rb" ]; then
      restore_rollback "$APP" "$app_rb" "$APP.failed-v8-$TS" \
    ...
    echo "DEPLOY=NO STOP=YES (rolled back to the pre-V8 generation)" >&2
    exit 1
  }

  # G1 source file pin (the ONE file v8 ships)
  ...
  # sudo/bash/tee command lines all contain "deploy-v8") is excluded; a
  # concurrent v8 double-run is already impossible past the G0 noclobber
  ...
  local g6b_hits
  g6b_hits="$(pgrep -fl 'trusted-cp-deploy-install|run-authorized-transaction|run-routing-install|scheduler-self-healing-deploy|wap-broker-only-deploy' 2>/dev/null | grep -v 'deploy-v8' || true)"
  if [ -n "$g6b_hits" ]; then
    printf '%s\n' "$g6b_hits"
  ...
  setup_pm_guard
  local next="$TRUSTED_ROOT/app.next-v8-$TS"
  build_app_next_v8 "$APP" "$SOURCE_ROOT" "$next" || fail "G9 app.next build failed"
  local built_man="$(manifest_of "$next")"
  [ "$built_man" = "$FROZEN_TARGET_APP_MANIFEST_SHA" ] || fail "G9 built app.next manifest != frozen v8 target ($built_man)"
  ...
  [ "$gw_sha" = "$FROZEN_BROKER_GATEWAY_SHA" ] || fail "G9 gateway.js drifted (v8 must not touch gateway)"
  local sched_sha="$(file_sha "$next/packages/scheduler/src/scheduler.js")"
  [ "$sched_sha" = "$FROZEN_SCHEDULER_JS_SHA" ] || fail "G9 scheduler.js drifted (v8 must not touch scheduler)"
  ...
  if ! swap_out="$(swap_generations "$APP" "$next" "$app_rb")"; then
    ...
    [ -d "$APP" ] || { echo "FATAL: swap failed and restore failed — pre-V8 generation preserved at $app_rb; manual recovery required" >&2; exit 1; }
    fail "APPLY swap failed (restored pre-V8 generation)"
  fi
  say "APPLY swapped: pre-V8 generation preserved at $app_rb"
  ...
  [ "$dep_sched" = "$FROZEN_SCHEDULER_JS_SHA" ] || post_swap_fail "deployed scheduler.js changed (v8 must not touch scheduler)"
  local dep_b1="$(file_sha "$APP/$BROKER_SELFOPS_REL")"
  [ "$dep_b1" = "$FROZEN_BROKER_SELFOPS_SHA" ] || post_swap_fail "deployed broker self-ops != frozen v8 target"
  local dep_b2="$(file_sha "$APP/$BROKER_GATEWAY_REL")"
  [ "$dep_b2" = "$FROZEN_BROKER_GATEWAY_SHA" ] || post_swap_fail "deployed broker gateway != frozen v8 target"
  ...
  pm_guard_clean || post_swap_fail "package manager invoked during deployment"
  {
    ...
    echo "V8_SCOPE=$BROKER_SELFOPS_REL + $BROKER_GATEWAY_REL ONLY"
    ...
    echo "ROLLBACK=mv $APP $APP.failed-v8-\$TS && mv $app_rb $APP && launchctl kickstart -k system/ai.agent-core.runtime"
  ...
  cp "$RB_TMP/deploy-receipt.txt" "$STATE_ROOT/receipts/deploy-v8.receipt" \
