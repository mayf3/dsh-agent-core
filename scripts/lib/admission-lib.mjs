export { WATCHDOG_PAYLOAD_SNAPSHOT } from './watchdog-payload-snapshot.mjs'
export {
  adaptLiveComposeForWatchdog, adaptLiveIntegrationForWatchdog, buildOverlaySeedBytes,
  narrowOverlayUniverse, WATCHDOG_LIVE_ADAPTER_POST_SHA, WATCHDOG_LIVE_ADAPTER_SHA, WATCHDOG_PINNED_LIVE_DEPENDENCIES,
} from './admission-overlay.mjs'
...
export const WATCHDOG_DELETE_PATHS = new Set([['packages/scheduler/src', 'watchdog.js'].join('/')])
 ...
 * receipts, and the sudo/root context. Live-tree adaptation and overlay
 * narrowing live in admission-overlay.mjs (structure gate B2); the reviewed
 * payload byte digests live in watchdog-payload-snapshot.mjs (B1) and are
 * re-exported here as the single import surface.
 */
