...
export async function runAdmissionSelftest({ ctx, main, git, sha256, repoRoot }) {
  ...
  const runtimePredecessor = '<?xml version="1.0" encoding="UTF-8"?>\n<plist version="1.0"><dict><key>HOME</key><string>/Users/authsvc</string></dict></plist>\n'
  writeFileSync(join(fx, 'LaunchDaemons', 'ai.agent-core.runtime.plist'), runtimePredecessor)
  for (const role of ['w1', 'w2']) writeFileSync(join(fx, 'LaunchDaemons', `ai.agent-core.scheduler-watchdog-${role}.plist`), `OLD-${role}\n`)
  mkdirSync(join(fx, 'rollback'), { recursive: true, mode: 0o700 })
  for (const name of ['ai.agent-core.runtime', 'ai.agent-core.scheduler-watchdog-w1', 'ai.agent-core.scheduler-watchdog-w2']) {
    writeFileSync(join(fx, 'rollback', `${name}.plist.preimage`), 'PARTIAL-UNCOMMITTED')
  }
  ...
  const w1Plist = readFileSync(join(fx, 'LaunchDaemons', 'ai.agent-core.scheduler-watchdog-w1.plist'), 'utf8')
  const routingReaderGid = Number(/SCHEDULER_ROUTING_READER_GID<\/key><string>(\d+)</.exec(w1Plist)[1])
  const incidentOwnerGid = Number(/SCHEDULER_INCIDENT_OWNER_GID<\/key><string>(\d+)</.exec(w1Plist)[1])
  ok(routingReaderGid === ctx.authsvcGid && incidentOwnerGid === runtimeReaderGid, 'watchdog plist gids split: routing reader=authsvc primary, incident owner=runtime reader')
