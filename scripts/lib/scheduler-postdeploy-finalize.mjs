 ...
 *
 * Ownership contract (post-merge closure B5 — aligned with the deployment
 * controller's own sources, never hardcoded gids):
 *   - canonical Scheduler store           -> authsvc:authsvc (its primary group)
 *   - incident state (incidents.json)     -> owner/resolver = the STATE DIRECTORY
 *       the controller prepared (preparePrivateRuntimeDirectory +
 *       normalizeLegacyIncidentStateFiles fchown it to the runtime-reader gid,
 *       staff=20 in production) — read back via statSync(stateDir), the same
 *       source watchdogInstall uses to fill __INCIDENT_OWNER_GID__.
 *   - routing/control receipts            -> root:wheel {0,0} control ownership
 *       (deployment-routing reconcileLegacyRoutingControlOwnership migrates
 *       legacy 601 -> control; scheduler-cp-rollback reads {0,0}).
 */
...
/**
 * The finalize readback gate: every evidence source read with its canonical
 * ownership contract. Exported so tests execute THIS path (gid-split fixture:
...
async function run() {
  ...
  const val = (name) => { const index = argv.indexOf(name); return index >= 0 ? argv[index + 1] : undefined }
  const sourceSha = val('--source-sha')
  const canaryAgentId = val('--canary-agent-id')
  ...
  const routingReceipt = readRoutingReceipt()
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  run().catch((error) => {
    process.stderr.write(`[postdeploy] FAILED: ${error?.message ?? error}\n`)
    process.exit(1)
  })
}
