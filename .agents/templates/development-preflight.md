...
> 模板来自 `docs/specs/AGENT_REPO_KNOWLEDGE_GOVERNANCE_V1.md` §17（status: accepted）。
...
INHERITED_DECISIONS =            # RELEVANT_DECISIONS + DECISION_DISPOSITION（PRESERVE/AMEND/SUPERSEDE）
RELATED_SPECS =
...
- `PRIMARY_GOVERNING_SPEC` 必须恰好一个；`UNRESOLVED_CONFLICTS != NONE` 时**不得开工实现**。
