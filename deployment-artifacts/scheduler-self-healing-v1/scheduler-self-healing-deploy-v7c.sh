#!/bin/bash
# =============================================================================
# scheduler-self-healing-deploy-v7c.sh — SCHEDULER_SELF_HEALING_FROM_FEISHU_V1
# Phase C APP-ONLY SEALED GENERATION **FOLLOW-UP** (fresh live baseline
# 6447676c…, TWO-FILE scope: broker job_disposition surface).
#
# WHY V7C EXISTS: post-V7 acceptance found the live broker manifest stale
# (no job_disposition operation). The 2026-09-19 07:56 fresh census against
# the CURRENT live generation 6447676c (post cfc2729-g2 20:44 + WAP broker-only
# 06:57) proved: packages/scheduler == cfc2729 EXACTLY (zero diffs; the poison
# fix stays live), and packages/broker differs in exactly 6 files — of which
# TWO are this spec's governed job_disposition surface and FOUR belong to
# other lanes (workflow-execute / workflow / registry / schema — UNTOUCHED).
# V7C ships exactly those TWO files as a sealed app generation swap:
#   packages/broker/src/capabilities/self-ops.js -> b302810c… (adds the
#     self_ops job_disposition operation; pure manifest, zero imports)
#   packages/broker/src/gateway.js               -> 4c341db4… (adds the
#     job_disposition readiness gate + operations entry; diff vs live is
#     EXACTLY those two hunks — no other-lane bytes)
# Both bytes are identical across 41f354d, cfc2729 and the reviewed worktree.
# The server-side handler already lives in the deployed scheduler
# (self-ops/index.js job_disposition -> createJobDisposition).
#
...
FROZEN_PREIMAGE_APP_MANIFEST_SHA="6447676c4db88d09e8b4d178d4ee41f0b533c1f078f3b3b5554b05aeadbaaa06"
FROZEN_TARGET_APP_MANIFEST_SHA="2d45fda740f4f8427cac6d953e2dd4fcbc2a36d1c40e70d1861c457678e29764"
...
STATE_ROOT="/usr/local/var/agent-core/scheduler-self-healing-v7c-apponly"
RB_TMP="/private/tmp/scheduler-self-healing-deploy-v7c-receipt"
...
PM_TRIPFILE=""
setup_pm_guard() {
  PM_GUARD_DIR="$(mktemp -d /tmp/scheduler-v7c-pm-shim.XXXXXX)"
...
build_app_next_v7c() { # $1=live_app  $2=source_root  $3=out_dir
  local live="$1" src="$2" out="$3"
  rm -rf "$out"
  cp -a "$live" "$out"
  cp "$src/capabilities/self-ops.js" "$out/$BROKER_SELFOPS_REL"
  cp "$src/gateway.js"               "$out/$BROKER_GATEWAY_REL"
  # cp creates root-owned files under sudo; the deployed generation must
  # keep the live ownership (runtime runs as authsvc)
  if [ "$(id -u)" = "0" ]; then
    chown authsvc:authsvc "$out/$BROKER_SELFOPS_REL" "$out/$BROKER_GATEWAY_REL"
  fi
}
...
fixture_selftest() {
  local S; S="$(mktemp -d /tmp/scheduler-v7c-fixture.XXXXXX)"
  setup_pm_guard
  ...
  echo old-gateway-no-jobdisposition > "$S/app/packages/broker/src/gateway.js"
  ...
  printf 'new-gateway-with-job_disposition\n' > "$S/srcroot/gateway.js"
  ...
  # than build_app_next_v7c) — live tree with exactly TWO files replaced —
  ...
  cp "$S/srcroot/gateway.js" "$etree/packages/broker/src/gateway.js"
  expected="$(manifest_of "$etree")"

  build_app_next_v7c "$S/app" "$S/srcroot" "$S/app.next" || { echo "FIXTURE=FAIL build"; exit 1; }
  ...
  grep -q 'job_disposition' "$S/app.next/$BROKER_GATEWAY_REL" || { echo "FIXTURE=FAIL gateway bytes"; exit 1; }
  ...
  rb="$(swap_generations "$S/app" "$S/app.next" "$S/app.rollback-v7c-test")" || { echo "FIXTURE=FAIL swap"; exit 1; }
  [ -d "$S/app" ] && [ -d "$S/app.rollback-v7c-test" ] || { echo "FIXTURE=FAIL swap layout"; exit 1; }
  [ "$(manifest_of "$S/app")" = "$built" ] || { echo "FIXTURE=FAIL post-swap identity"; exit 1; }
  # rollback restore (Owner §6)
  restore_rollback "$S/app" "$S/app.rollback-v7c-test" "$S/app.failed-v7c-test" || { echo "FIXTURE=FAIL restore"; exit 1; }
  ...
  echo "FIXTURE=PASS (pm-guard clean, manifest-exact two-file build, scheduler/pkg untouched, swap+restore round-trip OK)"
  rm -rf "$S"
}
...
production_main() {
  ...
  local AUTH_MARKER="$STATE_ROOT/receipts/deploy-v7c.auth"
  ...
  if ! ( set -o noclobber; printf 'packet=FREEZE_V7C\ntarget=%s\nauthorized_by=mayf3\ncreated=%s\n' \
  ...
  post_swap_fail() { # $1 = reason ; automatic rollback per Owner §6
    ...
    if [ "$rollback_done" = "0" ] && [ -n "$app_rb" ] && [ -d "$app_rb" ]; then
      restore_rollback "$APP" "$app_rb" "$APP.failed-v7c-$TS" \
    ...
    echo "DEPLOY=NO STOP=YES (rolled back to the pre-V7C generation)" >&2
    exit 1
  }

  # G1 source file pins (the TWO files v7c ships)
  [ -f "$SOURCE_ROOT/capabilities/self-ops.js" ] || fail "G1 source self-ops missing"
  [ -f "$SOURCE_ROOT/gateway.js" ] || fail "G1 source gateway missing"
  ...
  local s2="$(file_sha "$SOURCE_ROOT/gateway.js")"
  [ "$s2" = "$FROZEN_BROKER_GATEWAY_SHA" ] || fail "G1 source gateway pin mismatch ($s2)"
  grep -q 'job_disposition' "$SOURCE_ROOT/capabilities/self-ops.js" || fail "G1 source self-ops missing job_disposition"
  grep -q 'job_disposition' "$SOURCE_ROOT/gateway.js" || fail "G1 source gateway missing job_disposition"
  say "G1 source files bound (41f354d==cfc2729 broker job_disposition surface, TWO-file scope)"
  ...
  # sudo/bash/tee command lines all contain "deploy-v7c") is excluded; a
  # concurrent v7c double-run is already impossible past the G0 noclobber
  ...
  local g6b_hits
  g6b_hits="$(pgrep -fl 'trusted-cp-deploy-install|run-authorized-transaction|run-routing-install|scheduler-self-healing-deploy|wap-broker-only-deploy' 2>/dev/null | grep -v 'deploy-v7c' || true)"
  if [ -n "$g6b_hits" ]; then
    printf '%s\n' "$g6b_hits"
  ...
  setup_pm_guard
  local next="$TRUSTED_ROOT/app.next-v7c-$TS"
  build_app_next_v7c "$APP" "$SOURCE_ROOT" "$next" || fail "G9 app.next build failed"
  local built_man="$(manifest_of "$next")"
  [ "$built_man" = "$FROZEN_TARGET_APP_MANIFEST_SHA" ] || fail "G9 built app.next manifest != frozen v7c target ($built_man)"
  ...
  [ "$sched_sha" = "$FROZEN_SCHEDULER_JS_SHA" ] || fail "G9 scheduler.js drifted (v7c must not touch scheduler)"
  ...
  say "G9 sealed app.next built+verified (manifest $built_man; two-file scope; pm-guard clean)"
  ...
  if ! swap_out="$(swap_generations "$APP" "$next" "$app_rb")"; then
    ...
    [ -d "$APP" ] || { echo "FATAL: swap failed and restore failed — pre-V7C generation preserved at $app_rb; manual recovery required" >&2; exit 1; }
    fail "APPLY swap failed (restored pre-V7C generation)"
  fi
  say "APPLY swapped: pre-V7C generation preserved at $app_rb"
  ...
  [ "$dep_sched" = "$FROZEN_SCHEDULER_JS_SHA" ] || post_swap_fail "deployed scheduler.js changed (v7c must not touch scheduler)"
  local dep_b1="$(file_sha "$APP/$BROKER_SELFOPS_REL")"
  [ "$dep_b1" = "$FROZEN_BROKER_SELFOPS_SHA" ] || post_swap_fail "deployed broker self-ops != frozen v7c target"
  local dep_b2="$(file_sha "$APP/$BROKER_GATEWAY_REL")"
  [ "$dep_b2" = "$FROZEN_BROKER_GATEWAY_SHA" ] || post_swap_fail "deployed broker gateway != frozen v7c target"
  ...
  pm_guard_clean || post_swap_fail "package manager invoked during deployment"
  {
    ...
    echo "V7C_SCOPE=$BROKER_SELFOPS_REL + $BROKER_GATEWAY_REL ONLY"
    ...
    echo "ROLLBACK=mv $APP $APP.failed-v7c-\$TS && mv $app_rb $APP && launchctl kickstart -k system/ai.agent-core.runtime"
  ...
  cp "$RB_TMP/deploy-receipt.txt" "$STATE_ROOT/receipts/deploy-v7c.receipt" \
