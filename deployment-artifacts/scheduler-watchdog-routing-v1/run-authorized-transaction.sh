...
#  wrapper r4 — ONE_GLOBAL_PRODUCTION_TRANSACTION_OWNER: fresh gates, one
#  parent-owned global production lock across deploy → routing/config → runtime
#  reload/readiness, delegated child verification via transaction id + token,
#  reliable installer rc, exact artifact pins. Product semantics unchanged.)
#
# Sequence:
...
#     reviewed code head f72255d; the only non-docs delta beyond it is the
...
#     FAILS CLOSED; resolves VERIFIED_DEPLOY_SHA = exact fresh origin/main SHA)
#   → ACQUIRE wrapper transaction lock
#   → ACQUIRE GLOBAL production-deploy mutex ONCE as parent; freeze
#     PRODUCTION_TRANSACTION_ID + LOCK_OWNER_TOKEN (only token SHA in holder)
#   → STAGING (disposable clone at EXACTLY VERIFIED_DEPLOY_SHA, HEAD==sha gate)
#   → DEPLOY via trusted-cp-deploy-install.sh; child verifies inherited owner
#     and MUST NOT acquire/release the global lock
#   → ROUTING selftest → candidate → check → plan → apply; mutation children
#     verify the same inherited owner and MUST NOT acquire/release the lock
#   → RELOAD runtime → READINESS/READBACK while the SAME lock is still held
#   → RELEASE GLOBAL ONCE → release wrapper transaction lock → receipt
#
...
REVIEWED_CODE_SHA="f72255d4feddb6deb4f907cbe70cca547f9e240a"
...
global_holder_matches() {
  ...
  [ -f "$holder" ] || return 1
  grep -Fqx "pid=$$" "$holder" || return 1
  grep -Fqx "transaction_id=$PRODUCTION_TRANSACTION_ID" "$holder" || return 1
  grep -Fqx "token_sha256=$(sha256_text "$LOCK_OWNER_TOKEN")" "$holder" || return 1
}

acquire_global_deploy_lock() {
  [ -n "$PRODUCTION_TRANSACTION_ID" ] || gate "global transaction id" "unset"
  [ -n "$LOCK_OWNER_TOKEN" ] || gate "global lock owner token" "unset"
  mkdir -p "$(dirname "$GLOBAL_DEPLOY_LOCK_DIR")" 2>/dev/null || true
  if mkdir "$GLOBAL_DEPLOY_LOCK_DIR" 2>/dev/null; then
    printf 'pid=%s\nuid=%s\ntransaction_id=%s\ntoken_sha256=%s\ncmd=%s\nstarted=%s\n' \
      "$$" "$(id -u)" "$PRODUCTION_TRANSACTION_ID" "$(sha256_text "$LOCK_OWNER_TOKEN")" \
      "${GLOBAL_LOCK_CMD:-scheduler watchdog production transaction}" \
      "$(date -u +%Y-%m-%dT%H:%M:%SZ)" > "$GLOBAL_DEPLOY_LOCK_DIR/holder"
    pass "global production-deploy mutex acquired ONCE" "$GLOBAL_DEPLOY_LOCK_DIR transaction=$PRODUCTION_TRANSACTION_ID"
  else
...
assert_global_deploy_lock_owned() {
  local stage="${1:-unknown-stage}"
  global_holder_matches || gate "global transaction ownership" "holder mismatch before $stage — lock LEFT IN PLACE"
  pass "global transaction ownership verified" "$stage"
}

release_global_deploy_lock() {
  global_holder_matches || gate "global mutex release" "missing or holder mismatch — left in place"
  rm -f "$GLOBAL_DEPLOY_LOCK_DIR/holder"
  rmdir "$GLOBAL_DEPLOY_LOCK_DIR" || gate "global mutex release" "rmdir failed — inspect manually"
  pass "global production-deploy mutex released ONCE" "$PRODUCTION_TRANSACTION_ID"
}

begin_global_production_transaction() {
  [ -z "${PRODUCTION_TRANSACTION_ID:-}" ] || gate "parent transaction identity" "PRODUCTION_TRANSACTION_ID must start unset"
  [ -z "${LOCK_OWNER_TOKEN:-}" ] || gate "parent transaction identity" "LOCK_OWNER_TOKEN must start unset"
  PRODUCTION_TRANSACTION_ID="scheduler-watchdog-$(date -u +%Y%m%dT%H%M%SZ)-$$-$(/usr/bin/openssl rand -hex 16)"
  LOCK_OWNER_TOKEN="$(/usr/bin/openssl rand -hex 32)"
...
deploy_from_staging() {
  ...
  assert_global_deploy_lock_owned "deploy child exit"
  pass "deploy installer rc=0 — inherited parent lock remained intact"
...
DEPLOY() {
  local S="$1"
  assert_global_deploy_lock_owned "DEPLOY phase"
  rm -rf "$S"
...
ROUTING() {
  local S="$1"
  assert_global_deploy_lock_owned "ROUTING phase"
  ...
  assert_global_deploy_lock_owned "routing candidate"
  ...
  assert_global_deploy_lock_owned "routing apply"
  pass "routing receipt + protected-metadata readback complete under parent lock"
}

RELOAD() {
  assert_global_deploy_lock_owned "runtime reload"
  launchctl kickstart -k "$RUNTIME_LABEL" || gate "kickstart" 1
  local i
  for i in 1 2 3 4 5 6; do
    sleep 5
    assert_global_deploy_lock_owned "readiness attempt $i"
...
selftest() {
  ...
  bash -n "$repo_root/$DEPLOY_INSTALLER" || gate "selftest: deploy installer syntax" 1
  bash "$repo_root/$DEPLOY_INSTALLER" --lock-selftest >/dev/null || gate "selftest: deploy delegated-lock contract" 1
  node --check "$repo_root/$ROUTING_INSTALLER" >/dev/null || gate "selftest: routing installer syntax" 1
  pass "selftest: child lock-contract/syntax checks"
  ...
  GLOBAL_DEPLOY_LOCK_DIR="$scratch/global.lock"
  PRODUCTION_TRANSACTION_ID="tx-selftest-parent"
  LOCK_OWNER_TOKEN="token-selftest-parent"
  export PRODUCTION_TRANSACTION_ID LOCK_OWNER_TOKEN
  acquire_global_deploy_lock
  assert_global_deploy_lock_owned "selftest exact parent"
  if ( LOCK_OWNER_TOKEN="wrong" assert_global_deploy_lock_owned "selftest mismatch" ) 2>/dev/null; then
    gate "selftest: wrong owner token must FAIL_CLOSED" 1
  fi
  pass "selftest: inherited owner mismatch FAIL_CLOSED"
  [ -d "$GLOBAL_DEPLOY_LOCK_DIR" ] || gate "selftest: parent lock must survive child/mismatch checks" 1
  release_global_deploy_lock
  [ ! -e "$GLOBAL_DEPLOY_LOCK_DIR" ] || gate "selftest: exact parent release must remove lock" 1
  GLOBAL_DEPLOY_LOCK_DIR="$saved_global"
  PRODUCTION_TRANSACTION_ID="$saved_tx"
  LOCK_OWNER_TOKEN="$saved_token"
  export PRODUCTION_TRANSACTION_ID LOCK_OWNER_TOKEN
  ...
  assert_global_deploy_lock_owned "after successful deploy selftest"
  pass "selftest: INSTALLER_SUCCESS_PATH preserves parent transaction ownership"
  if ( STUB_RC=7 deploy_from_staging "$fx/staging" "$fxapp" ) 2>/dev/null; then
    gate "selftest: installer failure must FAIL_CLOSED" 1
  fi
  assert_global_deploy_lock_owned "after failed deploy selftest"
  pass "selftest: INSTALLER_FAILURE_FAIL_CLOSED and parent lock remains"
  release_global_deploy_lock
...
ROUTING "$STAGING_DIR"
RELOAD
assert_global_deploy_lock_owned "final transaction readback"
COMPLETED_TRANSACTION_ID="$PRODUCTION_TRANSACTION_ID"
release_global_deploy_lock
unset LOCK_OWNER_TOKEN
release_lock
echo "PRODUCTION_TRANSACTION_RECEIPT transaction_id=$COMPLETED_TRANSACTION_ID deployed_sha=$VERIFIED_DEPLOY_SHA status=COMPLETE"
