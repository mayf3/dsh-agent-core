 ...
 * successful-reply call in src/index.js adding
...
test('UX-SEAM-4: Router source delta is confined to the single success call site', async () => {
  ...
  const diff = execFileSync('git', ['diff', 'origin/main', '--', 'packages/agent-router/src/index.js'], {
  ...
  assert.deepEqual(removed, [], 'the ONLY removed line in Router product code is the old success call')
  const otherRouterFiles = execFileSync('git', ['diff', '--name-only', 'origin/main', '--', 'packages/agent-router/src/'], {
    cwd: new URL('../../..', import.meta.url).pathname,
    encoding: 'utf8',
  }).toString().trim()
  assert.equal(otherRouterFiles, 'packages/agent-router/src/index.js', 'no other Router source file changes')
})
