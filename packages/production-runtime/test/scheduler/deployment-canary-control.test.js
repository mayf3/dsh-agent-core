...
import { createRetainedPostdeployCanary, POSTDEPLOY_CANARY_AGENT_ID, postdeployCanaryMessage } from '../../src/scheduler/deployment-canary-control.js'
...
test('postdeploy control creates one retained, no-delivery, no-retry canary and replays idempotently', async (t) => {
  ...
  const input = { sourceSha: 'a'.repeat(40), agentId: POSTDEPLOY_CANARY_AGENT_ID, at, nowMs }
  ...
  assert.equal(first.job.payload.message, postdeployCanaryMessage(input.sourceSha))
})
...
test('postdeploy control rejects arbitrary identity and non-near-future schedules before store access', async () => {
  ...
  await assert.rejects(createRetainedPostdeployCanary(noStore, { sourceSha: 'a'.repeat(40), agentId: POSTDEPLOY_CANARY_AGENT_ID, at: new Date(nowMs + 600_000).toISOString(), nowMs }), /five minutes/)
})
