...
log('info', '[standalone] foundation: @larksuite/channel (batch delayMs=0, chatQueue disabled, stale-drop disabled, requireMention=true)')
...
channel.on({
  message: (msg) => {
    try {
      const ev = normalizedToIngressEvent(msg, { botIdentity: channel.getBotIdentity() })
      ...
      log('warn', `[standalone] dropped non-message event: ${error?.message ?? error}`)
    }
  },
