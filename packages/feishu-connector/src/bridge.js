...
export function normalizedToIngressEvent(msg, opts = {}) {
  ...
  return {
    ...
    mentioned,
    addressed,
    attachments: (msg.resources ?? []).map((resource) => resourceAttachment(resource, rawMessage.content, msg.rawContentType)),
    raw,
...
/**
 * Preserve the V0 IngressEvent text ABI while keeping the SDK as the only
 * wire normalizer. Non-text placeholders are derived solely from the SDK's
 * normalized type/resources. Post text is projected from normalized Markdown
 * (never from raw content); resource metadata remains attachment-only.
 */
function ingressText(msg) {
  switch (msg.rawContentType) {
    case 'text':
      return msg.content
    case 'post':
      return legacyPostTextFromNormalized(msg.content)
    case 'image':
      return '[image]'
    case 'file': {
...
function legacyPostTextFromNormalized(value) {
