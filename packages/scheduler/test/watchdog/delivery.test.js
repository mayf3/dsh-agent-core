...
import { buildIdempotentFeishuRequest, retryableOutboxIntents, stableNotificationText } from '../../src/watchdog/delivery.js'
...
test('T21/T26 ambiguous transport replay keeps the exact key, payload, and downstream idempotency coordinate', () => {
  ...
  assert.equal(after.body.uuid, key.slice(0, 50))
  assert.equal(after.providerKey.length, 50)
