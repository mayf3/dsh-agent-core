...
const SCAN_SECRET = 'scan-vwxyz-secret-9f3a'
 ...
 */
function stubTokenEndpoint({ registry = new Map(), respond } = {}) {
  const requests = []
...
function writeAuthConfig(root, overrides = {}) {
...
function makeRoot(t, { withAuthConfig = true, overrides = {} } = {}) {
...
async function deliver(base, { authorization, body } = {}) {
...
/** Rewrite the auth config at the standard path with overrides (kept 0600). */
function writeAuthConfigAt(root, overrides) {
