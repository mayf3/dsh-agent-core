/**
 * @agent-core/broker — error-detail sanitizer
 * (AGENT_CORE_FORUM_MODERATION_CAPABILITIES_V2 CTR-FMC-012).
 *
 * Sole owner of the caller-visible `detail` sanitization pipeline:
 *   1. the spec-policy layer (CTR-FMC-012 steps 1-3, case-insensitive
 *      Bearer/Basic scheme redaction, sensitive-keyed assignment redaction,
 *      opaque-run redaction);
 *   2. the pre-existing stronger redaction layer (password / api-key forms,
 *      scheme-prefixed Authorization values, >=40-char opaque runs) — kept
 *      verbatim and still effective after the spec layer (no security
 *      regression);
 *   3. truncation to the first 500 Unicode code points (code-point-correct).
 *
 * Pure functions; no transport or credential imports.
 */
...
// matched case-insensitively per V2).
