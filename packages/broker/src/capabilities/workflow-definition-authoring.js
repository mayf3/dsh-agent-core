...
export const workflowDefinitionAuthoringManifest = withTransportErrors({
  ...
  description: 'Create a workflow definition, create its draft version, replace the complete draft graph, or publish the version. svc-workflow remains authoritative for identity, Domain Owner authorization, graph validation and lifecycle. Resolve the domainId argument first via workflow_my_domains (choose a domain where caller_role is DOMAIN_OWNER).',
