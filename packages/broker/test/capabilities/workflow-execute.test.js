...
test('workflow_execute: create_instance operation freezes the svc-workflow create binding', () => {
  ...
  // AGENT_CORE_WORKFLOW_EXECUTION_CLASS_BROKER_V1 CTR-WECB-001/002: optional
  // closed enum, structurally validated broker-side before transport.
  ...
  // verbatim passthrough (not_domain_owner/idempotency_conflict pre-date this
  // companion; invalid_input is the single new table row).
...
// ─── AGENT_CORE_WORKFLOW_EXECUTION_CLASS_BROKER_V1 (ACC-WECB-001/002/003) ────

test('WEC companion: legacy create (absent executionClass) maps to a body with NO executionClass key — byte-identical legacy wire', async () => {
  const tokenServer = await startTokenServer()
  ...
  // the builder only adds keys whose arg !== undefined (transport bindRequest).
...
test('WEC companion: svc class error families preserve verbatim (not_domain_owner / invalid_input / idempotency_conflict)', async () => {
  ...
  for (const [domainId, [status, code, message, requestId]] of Object.entries(failures)) {
    ...
    assert.equal(result.ok, false)
    // Verbatim passthrough: same code, same HTTP status family, zero broker
    // rewriting/retry/classification (AGENT_CORE_WORKFLOW_BROKER_ERROR_PRESERVATION_V1).
    assert.equal(result.error.code, code)
    assert.equal(result.error.status, status)
    assert.equal(result.error.detail, message)
    assert.equal(result.error.requestId, requestId)
